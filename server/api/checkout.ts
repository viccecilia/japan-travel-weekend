...
export interface ManualPaymentGateway{markPending(orderId:string,amount:number):Promise<boolean>}
...
export class CheckoutEndpoint{
  ...
  async post(authorization:string|undefined,input:ServerCheckoutInput){
    ...
    if(input.paymentMethod==='bank_transfer'){if(!await this.manual.markPending(reserved.orderId,quote.amount)){await this.inventory.cancel(session,reserved.orderId);return {status:503,body:{error:'manual_payment_unavailable'}}}return {status:200,body:{...reserved,status:'pending_manual_review' as const}}}
