...
export type ServerCheckoutInput={departureId:string;seats:number;idempotencyKey:string;paymentMethod:'card'|'bank_transfer'};
...
export interface ManualPaymentGateway{markPending(orderId:string):Promise<boolean>}
...
export class CheckoutEndpoint{
  ...
  async post(authorization:string|undefined,input:ServerCheckoutInput){
    ...
    const expiresAt=new Date(this.now().getTime()+15*60*1000).toISOString();const reserved=await this.inventory.reserve(session,{departureId:input.departureId,seats:input.seats,idempotencyKey:input.idempotencyKey,expiresAt});if(!reserved)return {status:409,body:{error:'inventory_unavailable'}};
    if(input.paymentMethod==='bank_transfer'){if(!await this.manual.markPending(reserved.orderId)){await this.inventory.cancel(session,reserved.orderId);return {status:503,body:{error:'manual_payment_unavailable'}}}return {status:200,body:{...reserved,status:'pending_manual_review' as const}}}
    const quote=await this.pricing.quote(input.departureId,input.seats);if(!quote){await this.inventory.cancel(session,reserved.orderId);return {status:409,body:{error:'price_unavailable'}}}const payment=await this.card.create({orderId:reserved.orderId,amount:quote.amount,idempotencyKey:input.idempotencyKey});if(!payment){await this.inventory.cancel(session,reserved.orderId);return {status:503,body:{error:'card_payment_unavailable'}}}return {status:200,body:{...reserved,status:'requires_payment_action' as const,clientSecret:payment.clientSecret}}
  }
}
