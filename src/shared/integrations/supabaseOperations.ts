...
export type OperationsDriver = {
  ...
  service_role?: "driver" | "guide" | "driver_guide";
  public_phone?: string | null;
  employee_code?: string | null;
  employment_base?: string | null;
...
export type OperationsAccountDeletionRequest = {
  id: string;
  status: "requested" | "deferred_active_booking" | "reviewing";
  requestedAt: string;
  updatedAt: string;
};
export type OperationsCancellationRequest = {
  id: string;
  orderId: string;
  status: string;
  reasonCode: string;
...
export type OperationsStaffApplication = {
  id: string;
  accountId: string;
  email: string;
  applicantName: string;
  requestedRole: "driver" | "guide";
  status: string;
  reviewNote: string;
  createdAt: string;
  updatedAt: string;
};
export type OperationsStaffLeave = {
  id: string;
  accountId: string;
  email: string;
  displayName: string;
...
export type OperationsReferralCouponCounts = {
  total: number;
  pending: number;
  active: number;
  reserved: number;
...
export type OperationsReferralRelation = {
  id: string;
  created_at: string;
  ...
  order_amount_jpy: number | null;
  order_discount_jpy: number | null;
  trip_title: string | null;
  departs_at: string | null;
  ends_at: string | null;
  inviter_coupon_id: string | null;
  inviter_coupon_status: string | null;
  available_at: string | null;
  invitee_coupon_id: string | null;
  invitee_coupon_status: string | null;
};
export type OperationsReferralAlert = {
...
export type OperationsReferralSummary = {
  ...
  couponCounts: OperationsReferralCouponCounts;
  discountAmountJpy: number;
  integrity: {
    relationships: number;
    expectedCoupons: number;
    actualCoupons: number;
    missingPairs: number;
    orphanCoupons: number;
  };
  alerts: OperationsReferralAlert[];
...
export type OperationsProductRevision = {
...
export type OperationsMerchandising = {
  tripId: string;
  slug: string;
  title: string;
  ...
  localeReadiness: Record<string, boolean>;
  version: number;
};
export type OperationsEditableDeparture = {
  id: string;
  tripTitle: string;
  departsAt: string;
  endsAt: string;
...
export type OperationsCommissionPayout = {
  id: string;
  accountId: string;
  displayName: string;
  amountJpy: number;
...
export class SupabaseOperationsRepository {
  async loadPaymentMetrics(serviceDate: string) {
    if (!this.client)
      return {
        data: null as { paidOrders: number; paidAmountJpy: number } | null,
        error: "运营数据服务未配置",
      };
    const { data, error } = await this.client.rpc(
      "get_operations_payment_metrics",
      { p_service_date: serviceDate },
    );
    const row = (data as Array<Record<string, unknown>> | null)?.[0];
    return error || !row
      ? { data: null, error: error?.message ?? "付款统计读取失败" }
      : {
          data: {
            paidOrders: Number(row.paid_orders ?? 0),
            paidAmountJpy: Number(row.paid_amount_jpy ?? 0),
          },
          error: null,
        };
  }
  async loadSystemReleaseInfo() {
    if (!this.client)
      return {
        data: null as { migrationVersion: string; databaseTime: string } | null,
        error: "运营数据服务未配置",
      };
    const { data, error } = await this.client.rpc(
      "get_operations_system_release_info",
    );
    const row = (data as Array<Record<string, unknown>> | null)?.[0];
    return error || !row
      ? { data: null, error: error?.message ?? "数据库版本读取失败" }
      : {
          data: {
            migrationVersion: String(row.migration_version),
            databaseTime: String(row.database_time),
          },
          error: null,
        };
  }
  ...
  async listProducts() {
    ...
    const { data, error } = await this.client.rpc("get_operations_products");
    if (error) return { data: [] as OperationsProduct[], error: error.message };
    return {
      data: ((data ?? []) as Array<Record<string, unknown>>).map((row) => ({
        id: String(row.id),
        slug: String(row.slug),
        status: String(row.status) as OperationsProduct["status"],
        ...
        draftRevision:
          row.draft_revision == null ? null : Number(row.draft_revision),
  ...
  async saveProductDraft(input: {
    ...
    const { error } = await this.client.rpc("operations_save_product_draft", {
      p_trip: input.id,
      p_expected_catalog_version: input.expectedVersion,
      p_title: input.title,
  ...
  async publishProduct(id: string, expectedVersion: number) {
    if (!this.client) return { ok: false, error: "运营数据服务未配置" };
    const { error } = await this.client.rpc("operations_publish_product", {
      p_trip: id,
      p_expected_catalog_version: expectedVersion,
    });
    return { ok: !error, error: error?.message ?? null };
  }
  async createProduct(input: { slug: string; title: string }) {
    ...
    return {
      ok: !error,
      id: error ? null : String(data),
      error: error?.message ?? null,
    };
  }
  async copyProduct(input: {
    sourceId: string;
    ...
    const { data, error } = await this.client.rpc(
      "operations_copy_product_versioned",
      {
        p_source: input.sourceId,
        p_expected_catalog_version: input.sourceVersion,
        p_slug: input.slug,
        p_title: input.title,
      },
    );
  ...
  async setProductStatus(input: {
    ...
    if (!this.client) return { ok: false, error: "运营数据服务未配置" };
    const { error } = await this.client.rpc("operations_set_product_status", {
      p_trip: input.id,
      p_expected_catalog_version: input.expectedVersion,
      p_action: input.action,
      p_restore_revision: input.restoreRevision ?? null,
    });
    return { ok: !error, error: error?.message ?? null };
  }
  async listProductRevisions(id: string) {
    if (!this.client)
      return {
        data: [] as OperationsProductRevision[],
        error: "运营数据服务未配置",
      };
    const { data, error } = await this.client.rpc(
      "get_operations_product_revisions",
      { p_trip: id },
    );
    return {
      data: ((data ?? []) as Array<Record<string, unknown>>).map((row) => ({
  ...
  async uploadProductImage(productId: string, file: File) {
    if (!this.client)
      return { url: null as string | null, error: "运营数据服务未配置" };
    if (
      !["image/jpeg", "image/png", "image/webp"].includes(file.type) ||
      file.size > 10 * 1024 * 1024
    )
      return { url: null, error: "仅支持不超过10MB的 JPG、PNG 或 WebP" };
    ...
    return {
      url: this.client.storage.from("route-media").getPublicUrl(path).data
        .publicUrl,
      error: null,
    };
  }
  async listCommissionPayouts() {
    if (!this.client)
      return {
        data: [] as OperationsCommissionPayout[],
        error: "运营数据服务未配置",
      };
    const { data, error } = await this.client
      .from("commission_payout_requests")
      .select(
        "id,account_id,amount_jpy,week_start,status,requested_at,provider_reference,profiles!commission_payout_requests_account_id_fkey(display_name)",
      )
    ...
    return {
      data: ((data ?? []) as Array<Record<string, unknown>>).map((row) => {
        ...
        return {
          ...
          displayName: String(profile?.display_name ?? "未命名账户"),
  ...
  async reviewCommissionPayout(input: {
    id: string;
    action:
      "approve" | "mark_processing" | "mark_paid" | "fail" | "retry" | "cancel";
    note: string;
    providerReference?: string;
  }) {
    return this.transition("operations_review_commission_payout", {
      p_request: input.id,
      p_action: input.action,
      p_note: input.note,
  ...
  async updateDriverResource(input: {
    id: string;
    displayName: string;
    status: string;
    serviceRole: string;
    ...
    return this.transition("operations_update_driver_resource", {
      p_driver: input.id,
      p_display_name: input.displayName,
      p_status: input.status,
      p_service_role: input.serviceRole,
      p_public_phone: input.publicPhone,
      p_internal_phone: input.internalPhone,
      p_operations_note: input.operationsNote,
    });
  }
  async updateFleetVehicle(input: {
    id: string;
    status: string;
    color: string;
    photoUrl: string;
    ...
    return this.transition("operations_update_fleet_vehicle_v2", {
      p_vehicle: input.id,
      p_status: input.status,
      p_color: input.color,
      ...
      p_operations_note: input.operationsNote,
      p_sellable_capacity: input.sellableCapacity,
    });
  }
  async previewDepartureBatch(input: {
    tripId: string;
    start: string;
    end: string;
    weekdays: number[];
    ...
    const { data, error } = await this.client.rpc(
      "operations_preview_departure_batch",
      {
        p_trip: input.tripId,
        p_start: input.start,
        p_end: input.end,
  ...
  async createDepartureBatch(input: {
    ...
    const { data, error } = await this.client.rpc(
      "operations_create_departure_batch",
      {
  ...
  async listEditableDepartures(from?: string, to?: string) {
    if (!this.client)
      return {
        data: [] as OperationsEditableDeparture[],
        error: "运营数据服务未配置",
      };
    const params = from && to ? { p_from: from, p_to: to } : undefined;
    const { data, error } = await this.client.rpc(
      "get_operations_editable_departures",
      params,
    );
    return {
      data: ((data ?? []) as Array<Record<string, unknown>>).map((row) => ({
        ...
        committedSeats: Number(row.committed_seats ?? 0),
  ...
  async updateDeparture(input: {
    ...
    const { data, error } = await this.client.rpc(
      "operations_update_departure_v2",
      {
        p_departure: input.id,
        p_expected_version: input.expectedVersion,
        p_departs_at: input.departsAt,
        p_ends_at: input.endsAt,
        p_price: input.price,
        p_capacity: input.capacity,
        p_sales_open: input.salesOpenAt,
        p_sales_close: input.salesCloseAt,
        p_status: input.status,
        p_meeting_name: input.meetingName,
        p_meeting_address: input.meetingAddress,
        p_map_lat: input.mapLat,
  ...
  async cancelDeparture(id: string, expectedVersion: number, reason: string) {
    if (!this.client)
      return {
        ok: false,
        data: null as Record<string, unknown> | null,
  ...
  async loadDriverStatistics(from: string, to: string) {
    if (!this.client)
      return {
        data: [] as OperationsDriverStatistic[],
        error: "运营数据服务未配置",
      };
    const { data, error } = await this.client.rpc(
      "get_operations_driver_transport_statistics",
      { p_from: from, p_to: to },
    );
    return {
      data: ((data ?? []) as Array<Record<string, unknown>>).map((row) => ({
        ...
        serviceRole: String(row.service_role ?? "driver"),
        assignedRuns: Number(row.assigned_runs ?? 0),
        completedRuns: Number(row.completed_runs ?? 0),
        soldPassengers: Number(row.sold_passengers ?? 0),
        assignedPassengers: Number(row.assigned_passengers ?? 0),
        boardedPassengers: Number(row.boarded_passengers ?? 0),
        completedPassengers: Number(row.completed_passengers ?? 0),
        ...
        lastLocationAt: row.last_location_at
          ? String(row.last_location_at)
          : null,
      })),
      error: error?.message ?? null,
    };
  }
  async loadRunBoard(serviceDate: string) {
    if (!this.client)
      return { data: [] as OperationsRunRow[], error: "运营数据服务未配置" };
    const { data, error } = await this.client.rpc(
      "get_operations_daily_run_board",
      { p_service_date: serviceDate },
    );
    return {
      data: ((data ?? []) as Array<Record<string, unknown>>).map((row) => ({
        ...
        tripTitle: String(row.trip_title),
        departsAt: String(row.departs_at),
        departureStatus: String(row.departure_status),
        vehicleGroupId: row.vehicle_group_id
          ? String(row.vehicle_group_id)
          : null,
        ...
        journeyStatus: String(row.journey_status ?? "preparing"),
        currentStop: row.current_stop ? String(row.current_stop) : null,
        openIncidents: Number(row.open_incidents ?? 0),
  ...
  async listIncidents(input: {
    ...
    if (!this.client)
      return { data: [] as OperationsIncident[], error: "运营数据服务未配置" };
    let query = this.client
      .from("operational_incidents")
      .select(
        "id,departure_id,vehicle_group_id,order_id,kind,severity,status,summary,owner_id,resolution,created_at,updated_at",
      )
      .order("created_at", { ascending: false });
    if (input.departureId) query = query.eq("departure_id", input.departureId);
    if (input.vehicleGroupId)
      query = query.eq("vehicle_group_id", input.vehicleGroupId);
    const { data, error } = await query;
    return {
      data: ((data ?? []) as Array<Record<string, unknown>>).map((row) => ({
        id: String(row.id),
        departureId: row.departure_id ? String(row.departure_id) : null,
        vehicleGroupId: row.vehicle_group_id
          ? String(row.vehicle_group_id)
          : null,
  ...
  async resolveIncident(input: {
    id: string;
    status: "confirmed" | "in_progress" | "waiting_passenger" | "resolved";
    resolution: string;
  }) {
    return this.transition("operations_resolve_incident", {
      p_incident: input.id,
      p_status: input.status,
  ...
  async listMerchandising() {
    if (!this.client)
      return {
        data: [] as OperationsMerchandising[],
        error: "运营数据服务未配置",
      };
    const { data, error } = await this.client.rpc(
      "get_operations_merchandising",
    );
    return {
      data: ((data ?? []) as Array<Record<string, unknown>>).map((row) => ({
        tripId: String(row.trip_id),
        ...
        title: String(row.title),
        featuredRank:
          row.featured_rank == null ? null : Number(row.featured_rank),
        campaignKey: row.campaign_key ? String(row.campaign_key) : null,
        visibleFrom: row.visible_from ? String(row.visible_from) : null,
        visibleUntil: row.visible_until ? String(row.visible_until) : null,
        travelFrom: row.travel_from ? String(row.travel_from) : null,
        travelUntil: row.travel_until ? String(row.travel_until) : null,
        localeReadiness: (row.locale_readiness ?? {}) as Record<
          string,
          boolean
        >,
  ...
  async loadLinkCampaignReview() {
    ...
    const [campaigns, submissions] = await Promise.all([
      this.client
        .from("link_campaigns")
        .select(
          "id,campaign_month,status,scoring_rules,official_handles,opens_at,closes_at",
        )
  ...
  async listOrders(input: {
    ...
    if (!this.client)
      return { data: [] as OperationsOrder[], error: "运营数据服务未配置" };
    const { data, error } = await this.client.rpc("get_operations_orders", {
    ...
    return {
      data: ((data ?? []) as Array<Record<string, unknown>>).map((row) => ({
        ...
        discountAmountJpy: Number(row.discount_amount_jpy),
        ...
        refundedAmountJpy: Number(row.refunded_amount_jpy),
      })),
      error: error?.message ?? null,
    };
  }
    ...
    try {
      const [
        ...
        this.client
          .from("fleet_vehicles")
          .select(
            "id,registration_identifier,vehicle_type_key,external_dispatch_id,status,public_color,public_photo_url,model_name,inspection_required,operations_note,sellable_capacity",
          )
        ...
        this.client
          ...
          .order("display_name"),
        this.client.rpc(
          "get_operations_dashboard_departures",
          from && to ? { p_from: from, p_to: to } : undefined,
        ),
        this.client.rpc("get_operations_booking_drafts"),
        this.client
          .from("fulfilment_work_items")
          .select("id,order_id,departure_id,kind,status,created_at,updated_at")
        ...
        this.client
          ...
          .in("status", ["requested", "deferred_active_booking", "reviewing"])
          .order("requested_at", { ascending: true })
          .limit(100),
        this.client
          .from("order_cancellation_requests")
          .select(
            "id,order_id,status,reason_code,refund_percent,estimated_refund_amount,requested_at,updated_at",
          )
          .in("status", [
            "requested",
            "reviewing",
      ...
      const notificationDeliveryIssues = (
        (notificationIssues.data ?? []) as {
          id: string;
          event_type: string;
      ...
      const accountDeletionRequests = (
        (deletionRequests.data ?? []) as Array<{
          id: string;
          status: "requested" | "deferred_active_booking" | "reviewing";
          requested_at: string;
          updated_at: string;
      ...
      const cancellationQueue = (
        (cancellationRequests.data ?? []) as Array<{
          id: string;
          order_id: string;
          status: string;
          reason_code: string;
        ...
        refundPercent: Number(row.refund_percent),
        estimatedRefundAmount: Number(row.estimated_refund_amount),
        requestedAt: row.requested_at,
        updatedAt: row.updated_at,
      }));
      const staffApplicationQueue = (
        (staffApplications.data ?? []) as Array<{
          id: string;
          account_id: string;
          email: string;
          applicant_name: string;
          requested_role: "driver" | "guide";
          status: string;
          review_note: string;
          created_at: string;
          updated_at: string;
      ...
      const staffLeaveQueue = (
        (staffLeaveRequests.data ?? []) as Array<{
          id: string;
          account_id: string;
          email: string;
          display_name: string;
        ...
        createdAt: row.created_at,
        conflictingTasks: Number(row.conflicting_tasks),
      }));
  ...
  async createVehicle(input: {
    ...
    const { error } = await this.client.rpc("operations_create_vehicle_v2", {
      ...
      p_public_color: input.publicColor,
      p_public_photo_url: input.publicPhotoUrl || null,
    });
    return !error;
  }
  async saveRouteCatalog(input: {
    ...
    if (loadError || !current)
      return { ok: false, error: loadError?.message ?? "路线不存在" };
    ...
    if (!Object.keys(patch).length)
      return { ok: false, error: "没有可保存的修改" };
    const { error } = await this.client.rpc("operations_patch_route_catalog", {
      p_slug: input.slug,
      p_expected_version: Number(current.catalog_version),
      p_patch: patch,
    });
    return { ok: !error, error: error?.message ?? null };
  }
  async createDriver(input: {
    ...
    serviceRole: "driver" | "guide" | "driver_guide";
    publicPhone: string;
  }) {
    if (!this.client) return false;
  ...
  async resolveBankTransfer(input: {
    ...
    if (!this.client)
      return { ok: false, result: null, error: "运营数据服务未配置" };
    const { data, error } = await this.client.rpc(
      "operations_resolve_bank_transfer",
      {
  ...
  async reviewAccountDeletion(
    requestId: string,
    decision: "reviewing" | "rejected",
    note: string,
  ) {
    return this.transition("operations_review_account_deletion", {
      p_request: requestId,
      p_decision: decision,
      p_note: note,
    });
  ...
  async rejectCancellationRequest(requestId: string, reason: string) {
    return this.transition("operations_reject_cancellation_request", {
      p_request: requestId,
      p_reason: reason,
    });
  }
  async retryNotificationDelivery(outboxId: string, reason: string) {
    return this.transition("operations_retry_notification_delivery", {
      p_outbox: outboxId,
      p_reason: reason,
    });
  }
  async reviewStaffApplication(
    ...
    return this.transition("operations_review_staff_application", {
      p_application: applicationId,
      p_decision: decision,
      p_note: note,
    });
  }
  async reviewStaffLeave(
    requestId: string,
    decision: "approved" | "rejected",
    note: string,
  ) {
    return this.transition("operations_review_staff_leave", {
      p_request: requestId,
      p_decision: decision,
      p_note: note,
    });
  }
  async loadReferralSummary() {
    if (!this.client) return null;
    const { data, error } = await this.client.rpc(
      "get_operations_referral_summary",
    );
    return error ? null : (data as OperationsReferralSummary);
  }
  async updateReferralSettings(
    ...
    return this.transition("operations_update_referral_settings", {
      p_discount_percent: discountPercent,
      p_validity_days: validityDays,
      p_active: active,
    });
  }
  async setReferralCouponStatus(
    couponId: string,
    action: "freeze" | "void" | "restore",
    reason: string,
  ) {
    return this.transition("operations_set_referral_coupon_status", {
      p_coupon: couponId,
      p_action: action,
      p_reason: reason,
    });
