...
export class SupabaseOperationsRepository {
  ...
  async previewDepartureBatch(input:{tripId:string;start:string;end:string;weekdays:number[];departureTime:string;durationMinutes:number;price:number;capacity:number;salesOpen:string;closeHours:number}){if(!this.client)return {data:[] as Array<Record<string,unknown>>,error:'运营数据服务未配置'};const {data,error}=await this.client.rpc('operations_preview_departure_batch',{p_trip:input.tripId,p_start:input.start,p_end:input.end,p_weekdays:input.weekdays,p_departure_time:input.departureTime,p_duration_minutes:input.durationMinutes,p_price:input.price,p_capacity:input.capacity,p_sales_open:input.salesOpen,p_close_hours:input.closeHours});return {data:Array.isArray(data)?data as Array<Record<string,unknown>>:[],error:error?.message??null}}
  async createDepartureBatch(input:{operationId:string;tripId:string;start:string;end:string;weekdays:number[];departureTime:string;durationMinutes:number;price:number;capacity:number;salesOpen:string;closeHours:number;meetingName:string;meetingAddress:string}){if(!this.client)return {data:null,error:'运营数据服务未配置'};const {data,error}=await this.client.rpc('operations_create_departure_batch',{p_operation:input.operationId,p_trip:input.tripId,p_start:input.start,p_end:input.end,p_weekdays:input.weekdays,p_departure_time:input.departureTime,p_duration_minutes:input.durationMinutes,p_price:input.price,p_capacity:input.capacity,p_sales_open:input.salesOpen,p_close_hours:input.closeHours,p_meeting_name:input.meetingName,p_meeting_address:input.meetingAddress});return {data:data as Record<string,unknown>|null,error:error?.message??null}}
