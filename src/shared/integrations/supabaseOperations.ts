...
export type OperationsIncident={id:string;departureId:string|null;vehicleGroupId:string|null;orderId:string|null;kind:string;severity:string;status:string;summary:string;ownerId:string|null;resolution:string|null;createdAt:string;updatedAt:string};
...
export class SupabaseOperationsRepository {
  async loadPaymentMetrics(serviceDate:string){if(!this.client)return {data:null as {paidOrders:number;paidAmountJpy:number}|null,error:'运营数据服务未配置'};const {data,error}=await this.client.rpc('get_operations_payment_metrics',{p_service_date:serviceDate});const row=(data as Array<Record<string,unknown>>|null)?.[0];return error||!row?{data:null,error:error?.message??'付款统计读取失败'}:{data:{paidOrders:Number(row.paid_orders??0),paidAmountJpy:Number(row.paid_amount_jpy??0)},error:null}}
  async loadSystemReleaseInfo(){if(!this.client)return {data:null as {migrationVersion:string;databaseTime:string}|null,error:'运营数据服务未配置'};const {data,error}=await this.client.rpc('get_operations_system_release_info');const row=(data as Array<Record<string,unknown>>|null)?.[0];return error||!row?{data:null,error:error?.message??'数据库版本读取失败'}:{data:{migrationVersion:String(row.migration_version),databaseTime:String(row.database_time)},error:null}}
  ...
  async copyProduct(input:{sourceId:string;sourceVersion:number;slug:string;title:string}){if(!this.client)return {ok:false,id:null as string|null,error:'运营数据服务未配置'};const {data,error}=await this.client.rpc('operations_copy_product_versioned',{p_source:input.sourceId,p_expected_catalog_version:input.sourceVersion,p_slug:input.slug,p_title:input.title});return {ok:!error,id:error?null:String(data),error:error?.message??null}}
  ...
  async listEditableDepartures(from?:string,to?:string){if(!this.client)return {data:[] as OperationsEditableDeparture[],error:'运营数据服务未配置'};const params=from&&to?{p_from:from,p_to:to}:undefined;const {data,error}=await this.client.rpc('get_operations_editable_departures',params);return {data:((data??[]) as Array<Record<string,unknown>>).map(row=>({id:String(row.id),tripTitle:String(row.trip_title),departsAt:String(row.departs_at),endsAt:String(row.ends_at),price:Number(row.seat_price_jpy),capacity:Number(row.capacity),salesOpenAt:String(row.sales_open_at),salesCloseAt:String(row.sales_close_at),status:String(row.status),version:Number(row.schedule_version),committedSeats:Number(row.committed_seats??0),meetingName:String(row.meeting_name??''),meetingAddress:String(row.meeting_address??''),mapLat:Number(row.map_lat),mapLng:Number(row.map_lng),paidOrders:Number(row.paid_orders??0)})),error:error?.message??null}}
  ...
  async listIncidents(input:{departureId?:string;vehicleGroupId?:string}){if(!this.client)return {data:[] as OperationsIncident[],error:'运营数据服务未配置'};let query=this.client.from('operational_incidents').select('id,departure_id,vehicle_group_id,order_id,kind,severity,status,summary,owner_id,resolution,created_at,updated_at').order('created_at',{ascending:false});if(input.departureId)query=query.eq('departure_id',input.departureId);if(input.vehicleGroupId)query=query.eq('vehicle_group_id',input.vehicleGroupId);const {data,error}=await query;return {data:((data??[]) as Array<Record<string,unknown>>).map(row=>({id:String(row.id),departureId:row.departure_id?String(row.departure_id):null,vehicleGroupId:row.vehicle_group_id?String(row.vehicle_group_id):null,orderId:row.order_id?String(row.order_id):null,kind:String(row.kind),severity:String(row.severity),status:String(row.status),summary:String(row.summary),ownerId:row.owner_id?String(row.owner_id):null,resolution:row.resolution?String(row.resolution):null,createdAt:String(row.created_at),updatedAt:String(row.updated_at)})),error:error?.message??null}}
  async resolveIncident(input:{id:string;status:'confirmed'|'in_progress'|'waiting_passenger'|'resolved';resolution:string}){return this.transition('operations_resolve_incident',{p_incident:input.id,p_status:input.status,p_resolution:input.resolution,p_owner:null})}
