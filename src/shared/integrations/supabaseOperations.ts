...
export type OperationsEditableDeparture={id:string;tripTitle:string;departsAt:string;price:number;capacity:number;salesOpenAt:string;salesCloseAt:string;status:string;version:number;committedSeats:number};
export type OperationsDriverStatistic={driverId:string;driverName:string;assignedRuns:number;completedRuns:number;openIncidents:number;lastLocationAt:string|null};
...
export class SupabaseOperationsRepository {
  ...
  async listEditableDepartures(){if(!this.client)return {data:[] as OperationsEditableDeparture[],error:'运营数据服务未配置'};const {data,error}=await this.client.rpc('get_operations_editable_departures');return {data:((data??[]) as Array<Record<string,unknown>>).map(row=>({id:String(row.id),tripTitle:String(row.trip_title),departsAt:String(row.departs_at),price:Number(row.seat_price_jpy),capacity:Number(row.capacity),salesOpenAt:String(row.sales_open_at),salesCloseAt:String(row.sales_close_at),status:String(row.status),version:Number(row.schedule_version),committedSeats:Number(row.committed_seats??0)})),error:error?.message??null}}
  async updateDeparture(input:{id:string;expectedVersion:number;price:number;capacity:number;salesOpenAt:string;salesCloseAt:string;status:string}){if(!this.client)return {ok:false,error:'运营数据服务未配置'};const {error}=await this.client.rpc('operations_update_departure',{p_departure:input.id,p_expected_version:input.expectedVersion,p_price:input.price,p_capacity:input.capacity,p_sales_open:input.salesOpenAt,p_sales_close:input.salesCloseAt,p_status:input.status});return {ok:!error,error:error?.message??null}}
  async loadDriverStatistics(from:string,to:string){if(!this.client)return {data:[] as OperationsDriverStatistic[],error:'运营数据服务未配置'};const {data,error}=await this.client.rpc('get_operations_driver_statistics',{p_from:from,p_to:to});return {data:((data??[]) as Array<Record<string,unknown>>).map(row=>({driverId:String(row.driver_id),driverName:String(row.driver_name),assignedRuns:Number(row.assigned_runs??0),completedRuns:Number(row.completed_runs??0),openIncidents:Number(row.open_incidents??0),lastLocationAt:row.last_location_at?String(row.last_location_at):null})),error:error?.message??null}}
