...
export class SupabaseOrderRepository {
  ...
  async loadOwnOrders() {
    ...
    const { data, error } = await this.client
      .from("orders")
      .select("id,departure_id,seat_count,status,amount,currency,manual_payment_due_at,created_at,departure:departures(departs_at,status,trip:trips(slug))")
      .order("created_at", { ascending: false });
    return error
      ? { data: [], error: "订单读取失败" }
  ...
  async loadOwnShareCampaign(){if(!this.client)return {campaign:null,submissions:[],orders:[],error:'分享活动服务未配置'};const campaigns=await this.client.from('link_campaigns').select('id,campaign_month,status,opens_at,closes_at,rules_version').eq('status','open').order('campaign_month',{ascending:false}).limit(1);const campaign=campaigns.data?.[0]??null;const [submissions,orders]=await Promise.all([this.client.from('link_campaign_submissions').select('id,campaign_id,order_id,platform,post_url,platform_account,status,created_at,campaign:link_campaigns(campaign_month)').order('created_at',{ascending:false}),campaign?this.client.rpc('get_own_travel_moment_orders',{p_campaign:String(campaign.id)}):Promise.resolve({data:[],error:null})]);const error=campaigns.error??submissions.error??orders.error;return {campaign,submissions:(submissions.data??[]).map((item:any)=>({...item,campaignMonth:item.campaign?.campaign_month??null})),orders:(orders.data??[]).map((item:any)=>({id:item.order_id,tripTitle:item.trip_title,departureAt:item.departure_at})),error:error?'分享活动读取失败':null}}
  async submitShareLink(input:{campaignId:string;orderId:string;platform:'tiktok'|'instagram';url:string;platformAccount:string;authorizationVersion:string;mentionConfirmed:boolean;authorized:boolean}){if(!input.mentionConfirmed||!input.authorized)return {ok:false,error:'请先确认 @ 官方账号及指定帖授权'};if(!this.client)return {ok:false,error:'分享活动服务未配置'};const {data,error}=await this.client.rpc('submit_travel_share_link',{p_campaign:input.campaignId,p_order:input.orderId,p_platform:input.platform,p_url:input.url,p_platform_account:input.platformAccount,p_authorization_version:input.authorizationVersion,p_authorization_scope:{repost:true,display:true,monitoring:true,mention_confirmed:input.mentionConfirmed,authorized:input.authorized,download:false,editing:false,reupload:false,paid_ads:false}});return error||!data?{ok:false,error:error?.message??'投稿未提交'}:{ok:true,error:null,id:String(data)}}
}
