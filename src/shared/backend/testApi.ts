...
export class TestBackendApi{
  ...
  resumePayment(input:{orderId:string;idempotencyKey:string}){return this.post<{orderId:string;amount:number;clientSecret:string;status:'requires_payment_action'}>('/v1/payments/resume',input)}
