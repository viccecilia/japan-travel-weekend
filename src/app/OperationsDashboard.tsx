        ...
        snapshot && (
          <>
            ...
            {referral&&<section id="referral-monitor" className="operations-section operations-referral-monitor">
              <header><div><span>增长、结算与风控</span><h2>推荐归因与历史优惠券</h2></div><small>当前现金规则：新人首笔符合资格订单的基础票价 10%，行程完成且无退款争议后解锁，每个日本自然周最多提现一次</small></header>
              ...
              <div className="operations-command-grid">
                ...
                <div><h3>推荐关系明细</h3>{referral.relations.length===0?<p className="operations-empty">暂无推荐关系。</p>:<div className="operations-dispatch-list operations-referral-list">{referral.relations.map(row=><article key={row.id}><div><b>{row.inviter_name||row.inviter_email} → {row.invitee_name||row.invitee_email}</b><span>首单现金佣金 10%</span></div><span>{row.trip_title??'尚未购买行程'} · {row.order_status??'尚未付款'}</span><small>新人券：{row.invitee_coupon_status??'缺失'} · 历史推荐人券：{row.inviter_coupon_status??'无'}{row.available_at?` · 历史券原定 ${japanDate(row.available_at)} 解锁`:''}</small><small>订单 ¥{row.order_amount_jpy??0} · 优惠 ¥{row.order_discount_jpy??0} · 推荐码 {row.referral_code}</small></article>)}</div>}<p className="operations-hint">历史推荐人优惠券仅用于核对存量权益，不再作为新推荐统计或现金佣金依据。</p></div>
              </div>
            </section>}
            ...
            <section id="resource-registry" className="operations-section">
              ...
              <div className="operations-registry-grid">
                <div><h3>司导人员</h3><div className="operations-dispatch-list">{snapshot.drivers.map(driver=><article key={driver.id}><div><b>{driver.display_name}</b><span>{driver.employee_code?`编号 ${driver.employee_code}`:"编号待补"}</span></div><span>{driver.employment_base??"所属待补"} · {driver.private_phone??"电话待补"}</span><small>{driver.account_id?"已绑定登录账户":"尚未绑定登录账户"} · {driver.status}{driver.operations_note?` · ${driver.operations_note}`:''}</small><button disabled={busy} onClick={()=>void editDriver(driver)}>编辑司导资料</button></article>)}</div></div>
                <div><h3>运营车辆</h3><div className="operations-dispatch-list">{snapshot.vehicles.map(vehicle=><article key={vehicle.id}><div><b>{vehicle.registration_identifier}</b><span>{vehicle.model_name??"车型待补"}</span></div><span>{vehicle.public_color??"颜色待补"} · {vehicle.vehicle_type_key==="unclassified-manual"?"座位数待确认":vehicle.vehicle_type_key}</span><small>{vehicle.inspection_required?"需要车检，禁止派单":vehicle.status}{vehicle.operations_note?` · ${vehicle.operations_note}`:""}</small><button disabled={busy} onClick={()=>void editVehicle(vehicle)}>编辑车辆资料</button></article>)}</div></div>
              </div>
            </section>
