...
type Props={markers:MapMarker[];activeMarkerId?:string|null;onMarkerClick?:(marker:MapMarker)=>void;onMapClick?:(point:Point)=>void;interactive:boolean;emptyLabel:string;unavailableLabel:string};
...
export function MapSurface({markers,activeMarkerId,onMarkerClick,onMapClick,interactive,emptyLabel,unavailableLabel}:Props){
 const host=useRef<HTMLDivElement>(null);const map=useRef<any>(null);const nativeMarkers=useRef<any[]>([]);const [status,setStatus]=useState<'loading'|'ready'|'unavailable'>(interactive?'loading':'unavailable');
 useEffect(()=>{let cancelled=false;if(!interactive){setStatus('unavailable');return}void loadGoogleMaps().then(()=>{if(cancelled||!host.current)return;map.current=new window.google.maps.Map(host.current,{center:markers[0]?{lat:markers[0].lat,lng:markers[0].lng}:{lat:35.0116,lng:135.7681},zoom:markers.length?14:11,disableDefaultUI:true,zoomControl:true,gestureHandling:'greedy'});map.current.addListener('click',(event:any)=>onMapClick?.({lat:event.latLng.lat(),lng:event.latLng.lng()}));setStatus('ready')}).catch(()=>!cancelled&&setStatus('unavailable'));return()=>{cancelled=true;nativeMarkers.current.forEach(marker=>marker.setMap(null));nativeMarkers.current=[]};},[interactive]);
 useEffect(()=>{if(status!=='ready'||!map.current)return;nativeMarkers.current.forEach(marker=>marker.setMap(null));nativeMarkers.current=markers.map(marker=>{const native=new window.google.maps.Marker({map:map.current,position:{lat:marker.lat,lng:marker.lng},title:marker.label,label:{text:mapMarkerGlyph[marker.type],color:'#17242c',fontWeight:'700'}});native.addListener('click',()=>onMarkerClick?.(marker));return native});const target=markers.find(marker=>marker.id===activeMarkerId)??markers[0];if(target)map.current.panTo({lat:target.lat,lng:target.lng})},[markers,status,activeMarkerId,onMarkerClick]);
