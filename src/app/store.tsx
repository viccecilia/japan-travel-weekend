...
import {createContext,useContext,useMemo,useState,type ReactNode} from 'react'; import type {Booking,DemoOrder,DemoState} from '../shared/types';import {initialTripRoomMessages} from '../shared/data/operations';
const KEY='jtw-demo-state-v1';
export const initialState:DemoState={user:null,booking:null,orders:[],completedTrips:1,credits:800,ownReferralCode:'WEEKEND-DEMO',language:'en',ui:{compact:false},tripRoom:{sharingLocation:false,messages:initialTripRoomMessages}};
const load=()=>{try{const saved=JSON.parse(localStorage.getItem(KEY)??'') as DemoState;return {...initialState,...saved,tripRoom:saved.tripRoom??initialState.tripRoom}}catch{return initialState}};
