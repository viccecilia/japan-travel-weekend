...
import type { ChildSeatChoice, Trip, TripSpotVideo, TripTimelineItem } from "../shared/types";
...
const localizedPublishedTimeline=(locale:PassengerLocale,trip:typeof trips[number])=>{
 const localeContent=trip.localizedContent?.[locale]; const translated=localeContent&&typeof localeContent.itinerary==='object'&&localeContent.itinerary!==null&&!Array.isArray(localeContent.itinerary)?localeContent.itinerary as Record<string,Record<string,unknown>>:{};
 return trip.timeline.map(item=>{const row=item.id?translated[item.id]??{}:{};return {...item,title:typeof row.stop_title==='string'&&row.stop_title.trim()?row.stop_title:item.title,subtitle:typeof row.subtitle==='string'&&row.subtitle.trim()?row.subtitle:item.subtitle,detail:typeof row.shortDescription==='string'&&row.shortDescription.trim()?row.shortDescription:typeof row.description==='string'&&row.description.trim()?row.description:item.detail,shortDescription:typeof row.shortDescription==='string'&&row.shortDescription.trim()?row.shortDescription:item.shortDescription,longDescription:typeof row.longDescription==='string'&&row.longDescription.trim()?row.longDescription:item.longDescription,videoLabel:typeof row.videoLabel==='string'&&row.videoLabel.trim()?row.videoLabel:item.videoLabel,tip:typeof row.tip==='string'&&row.tip.trim()?row.tip:item.tip};});
};
...
const passengerNavSection=(pathname:string):PassengerNavSection=>{
  ...
  if(pathname.startsWith('/app/messages')||pathname.startsWith('/app/my-trip')||pathname.startsWith('/app/ai-guide')||pathname.startsWith('/app/support'))return 'messages';
  return 'profile';
};
export function AppShell({
  ...
  return (
    <div className="app-stage">
      <div className={`app-frame passenger-v2 screen-${screen}${isDiscover?' discover-frame':''}`}>
...
function DiscoverCharters({kind}:{kind:'vip'|'groups'}) {
  const {state}=useApp();
  ...
  return <section className="discover-charters">
    {kind==='vip'?<Link className="passenger-vip-card" to="/app/vip-charter"><div><span>{vip.eyebrow}</span><h2>{vip.title}</h2><p>{vip.text}</p><small>{vip.features}</small><strong>{vip.action}</strong></div><img src="/images/vip-alphard-hotel.jpg" alt=""/></Link>:<Link className="passenger-private-card" to="/app/private-groups"><div><span>PRIVATE GROUPS</span><b>{h.privateTitle}</b><p>{h.privateText}</p></div><strong>{h.inquire} →</strong></Link>}
  </section>;
}
...
export function AppTrips() {
  ...
  return (
    <div className="route-catalog">
      <DiscoverCharters kind="vip"/>
      <h1>{discoverLabels[state.ui.locale??'zh-CN'].trips}</h1>
      ...
      <DiscoverCharters kind="groups"/>
    </div>
  );
}
const routeV2Copy: Record<PassengerLocale, {today:string; schedule:string; fees:string; notes:string; included:string; excluded:string; preparation:string; details:string; hide:string; departure:string; returning:string; minutes:string; book:string; booking:string; cancellation:string; participants:string; weather:string; baggage:string; safety:string; ready:string}> = {
  'zh-CN':{today:'今日行程',schedule:'详细时间轴',fees:'费用与准备',notes:'出行须知',included:'已包含',excluded:'未包含',preparation:'出发准备',details:'查看详细介绍 ↓',hide:'收起详细介绍 ↑',departure:'出发',returning:'预计返回',minutes:'分钟',book:'查看班次',booking:'预订须知',cancellation:'取消与退款',participants:'参加规则',weather:'天气提醒',baggage:'行李说明',safety:'安全提示',ready:'路线已经看清楚？再选择适合你的出发班次。'},
  'zh-TW':{today:'今日行程',schedule:'詳細時間軸',fees:'費用與準備',notes:'出行須知',included:'已包含',excluded:'未包含',preparation:'出發準備',details:'查看詳細介紹 ↓',hide:'收起詳細介紹 ↑',departure:'出發',returning:'預計返回',minutes:'分鐘',book:'查看班次',booking:'預訂須知',cancellation:'取消與退款',participants:'參加規則',weather:'天氣提醒',baggage:'行李說明',safety:'安全提示',ready:'行程已了解？選擇適合你的出發班次。'},
  ja:{today:'本日の行程',schedule:'詳しい行程',fees:'料金と準備',notes:'ご案内',included:'含まれるもの',excluded:'含まれないもの',preparation:'持ち物・準備',details:'詳しく見る ↓',hide:'閉じる ↑',departure:'出発',returning:'到着予定',minutes:'分',book:'出発日を選ぶ',booking:'予約について',cancellation:'取消・返金',participants:'参加条件',weather:'天候について',baggage:'手荷物について',safety:'安全について',ready:'行程を確認したら、出発日を選びましょう。'},
  en:{today:"Today's route",schedule:'Detailed timeline',fees:'What is included',notes:'Travel notes',included:'Included',excluded:'Not included',preparation:'Prepare',details:'Read more ↓',hide:'Show less ↑',departure:'Depart',returning:'Estimated return',minutes:'min',book:'View departures',booking:'Booking notes',cancellation:'Cancellation and refund',participants:'Participant rules',weather:'Weather',baggage:'Baggage',safety:'Safety',ready:'Ready to go? Choose a departure that suits you.'},
  ko:{today:'오늘의 일정',schedule:'상세 일정',fees:'요금과 준비',notes:'여행 안내',included:'포함 사항',excluded:'불포함 사항',preparation:'준비물',details:'자세히 보기 ↓',hide:'접기 ↑',departure:'출발',returning:'예상 귀환',minutes:'분',book:'출발편 보기',booking:'예약 안내',cancellation:'취소 및 환불',participants:'참가 규정',weather:'날씨 안내',baggage:'수하물 안내',safety:'안전 안내',ready:'일정을 확인했다면 출발편을 선택하세요.'},
  es:{today:'Ruta de hoy',schedule:'Itinerario detallado',fees:'Costes y preparación',notes:'Notas de viaje',included:'Incluido',excluded:'No incluido',preparation:'Preparación',details:'Ver detalles ↓',hide:'Ocultar detalles ↑',departure:'Salida',returning:'Regreso estimado',minutes:'min',book:'Ver salidas',booking:'Notas de reserva',cancellation:'Cancelación y reembolso',participants:'Reglas de participación',weather:'Clima',baggage:'Equipaje',safety:'Seguridad',ready:'¿Todo claro? Elige la salida que más te convenga.'},
  vi:{today:'Lịch trình hôm nay',schedule:'Dòng thời gian chi tiết',fees:'Chi phí và chuẩn bị',notes:'Lưu ý hành trình',included:'Đã bao gồm',excluded:'Chưa bao gồm',preparation:'Chuẩn bị',details:'Xem chi tiết ↓',hide:'Thu gọn ↑',departure:'Khởi hành',returning:'Về dự kiến',minutes:'phút',book:'Xem chuyến đi',booking:'Lưu ý đặt chỗ',cancellation:'Hủy và hoàn tiền',participants:'Quy định tham gia',weather:'Thời tiết',baggage:'Hành lý',safety:'An toàn',ready:'Đã rõ lịch trình? Hãy chọn chuyến phù hợp.'},
  ne:{today:'आजको यात्रा',schedule:'विस्तृत तालिका',fees:'लागत र तयारी',notes:'यात्रा सूचना',included:'समावेश',excluded:'समावेश छैन',preparation:'तयारी',details:'विस्तार हेर्नुहोस् ↓',hide:'लुकाउनुहोस् ↑',departure:'प्रस्थान',returning:'अनुमानित फिर्ता',minutes:'मिनेट',book:'प्रस्थान हेर्नुहोस्',booking:'बुकिङ सूचना',cancellation:'रद्द र फिर्ता',participants:'सहभागी नियम',weather:'मौसम',baggage:'सामान',safety:'सुरक्षा',ready:'यात्रा बुझ्नुभयो? आफ्नो प्रस्थान छान्नुहोस्।'},
};
const routeV2MediaCopy: Record<PassengerLocale, {play:string; unavailable:string; seconds:(value:number)=>string}> = {
  'zh-CN': {play:'播放视频', unavailable:'视频暂时无法播放，景点图文仍可正常查看。', seconds:value=>`${value} 秒`},
  'zh-TW': {play:'播放影片', unavailable:'影片暫時無法播放，仍可查看景點圖文。', seconds:value=>`${value} 秒`},
  ja: {play:'動画を再生', unavailable:'動画は一時的に再生できません。景点の案内はご覧いただけます。', seconds:value=>`${value} 秒`},
  en: {play:'Play video', unavailable:'This video is temporarily unavailable. The route information remains available.', seconds:value=>`${value}s`},
  ko: {play:'동영상 재생', unavailable:'동영상을 일시적으로 재생할 수 없습니다. 여행 안내는 계속 볼 수 있습니다.', seconds:value=>`${value}초`},
  es: {play:'Reproducir video', unavailable:'El video no está disponible temporalmente. La información del recorrido sigue disponible.', seconds:value=>`${value} s`},
...
function RouteDetailV2({trip, locale, selectedDeparture}: {trip: Trip; locale: PassengerLocale; selectedDeparture: {id:string;departureTime?:string|null;expectedEndTime?:string|null}|null}) {
  const copy=routeV2Copy[locale]; const local=trip.localizedContent?.[locale]??trip.localizedContent?.['zh-CN']??{}; const text=(key:string, fallback='')=>v2Text(local[key],fallback);
  const localizedList=(key:'highlights'|'included'|'excluded'|'preparation'|'notices', fallback:string[])=>localizedRouteList(local, trip.translationListItems?.[key]??[], key, fallback);
  ...
  const copyNotice=(key:string,fallback:string)=>text(key,fallback); const heroTitle=text('heroTitle',trip.heroTitle||text('title',trip.shortTitle)); const heroSubtitle=text('heroSubtitle',trip.heroSubtitle||trip.summary); const highlight=text('heroHighlightPhrase',trip.heroHighlightPhrase||''); const titleParts=highlight&&heroTitle.includes(highlight)?heroTitle.split(highlight):[heroTitle]; const timeline=localizedPublishedTimeline(locale,trip); const routeStops=timeline.map(item=>item.title).filter(Boolean); const start=text('departureCity',trip.departureCity||trip.region); const journey=[start,...routeStops,start].filter(Boolean); const departureTime=selectedDeparture?.departureTime?new Intl.DateTimeFormat(locale,{timeZone:'Asia/Tokyo',hour:'2-digit',minute:'2-digit'}).format(new Date(selectedDeparture.departureTime)):''; const returnTime=selectedDeparture?.expectedEndTime?new Intl.DateTimeFormat(locale,{timeZone:'Asia/Tokyo',hour:'2-digit',minute:'2-digit'}).format(new Date(selectedDeparture.expectedEndTime)):''; const travelNotes=[['bookingNotice',copy.booking,copyNotice('bookingNotice',trip.bookingNotice||trip.notices[0]||'')],['cancellationPolicy',copy.cancellation,copyNotice('cancellationPolicy',trip.cancellationPolicy||'')],['participantRules',copy.participants,copyNotice('participantRules',trip.participantRules||trip.childPolicy||'')],['weatherNotice',copy.weather,copyNotice('weatherNotice',trip.weatherNotice||trip.weatherPolicy||'')],['baggageNotice',copy.baggage,copyNotice('baggageNotice',trip.baggageNotice||trip.luggagePolicy||'')],['safetyNotice',copy.safety,copyNotice('safetyNotice',trip.safetyNotice||'')]] as const;
  return <div className="route-detail-v2"><RouteV2Hero trip={trip} heroTitle={heroTitle} heroSubtitle={heroSubtitle} titleParts={titleParts} highlight={highlight} start={start} mediaCopy={routeV2MediaCopy[locale]}/><section className="route-v2-section"><span className="route-v2-kicker">TODAY'S ROUTE</span><h2>{copy.today}</h2><div className="route-v2-chain"><div>{journey.map((item,index)=><span key={`${item}-${index}`}>{index>0&&<i>→</i>}{item}</span>)}</div>{(departureTime||returnTime)&&<small>{departureTime&&`${departureTime} ${copy.departure}`}{departureTime&&returnTime&&' · '}{returnTime&&`${copy.returning} ${returnTime}`}</small>}</div></section><section className="route-v2-section route-v2-spots">{timeline.map((spot,index)=><RouteV2Spot key={spot.id??`${spot.title}-${index}`} spot={spot} index={index} copy={copy} mediaCopy={routeV2MediaCopy[locale]} fallbackImage={trip.heroImage}/>)}</section><section className="route-v2-section"><span className="route-v2-kicker">DAY SCHEDULE</span><h2>{copy.schedule}</h2><div className="route-v2-timeline">{timeline.map((item,index)=><article key={item.id??`${item.title}-${index}`}><time>{item.time||'—'}</time><i/><div><b>{item.title}</b><p>{[item.detail,item.stayMinutes?`${item.stayMinutes} ${copy.minutes}`:'',item.type].filter(Boolean).join(' · ')}</p></div></article>)}</div></section>{(included.length||excluded.length||preparation.length>0)&&<section className="route-v2-section"><span className="route-v2-kicker">BEFORE YOU GO</span><h2>{copy.fees}</h2><div className="route-v2-info-grid">{included.length>0&&<RouteV2List title={copy.included} items={included}/>} {excluded.length>0&&<RouteV2List title={copy.excluded} items={excluded}/>} {preparation.length>0&&<RouteV2List title={copy.preparation} items={preparation}/>}</div></section>}{travelNotes.some(([, ,body])=>body)&&<section className="route-v2-section"><span className="route-v2-kicker">TRAVEL NOTES</span><h2>{copy.notes}</h2><div className="route-v2-notices">{travelNotes.filter(([, ,body])=>body).map(([key,label,body])=><details key={key}><summary>{label}<span>＋</span></summary><p>{body}</p></details>)}</div></section>}<section className="route-v2-cta"><small>READY WHEN YOU ARE</small><h3>{copy.ready}</h3><Link to={`/app/booking/${trip.slug}${selectedDeparture?`?departureId=${encodeURIComponent(selectedDeparture.id)}`:''}`}>{copy.book}</Link></section></div>;
}
function RouteV2Hero({trip,heroTitle,heroSubtitle,titleParts,highlight,start,mediaCopy}:{trip:Trip;heroTitle:string;heroSubtitle:string;titleParts:string[];highlight:string;start:string;mediaCopy:(typeof routeV2MediaCopy)['zh-CN']}) { const [play,setPlay]=useState(false); const [failed,setFailed]=useState(false); const video=trip.heroVideo; return <section className="route-v2-hero" style={{backgroundImage:`url(${trip.heroImage})`}}>{video?.url&&play&&!failed&&<video className="route-v2-hero-video" controls autoPlay muted playsInline poster={video.posterUrl||trip.heroImage} onError={()=>setFailed(true)}><source src={video.url} type="video/mp4"/></video>}<div className="route-v2-hero-copy"><span className="route-v2-kicker">DAY TRIP · {start}</span><h1>{titleParts.length===1?heroTitle:<>{titleParts[0]}<em>{highlight}</em>{titleParts.slice(1).join(highlight)}</>}</h1>{heroSubtitle&&<p>{heroSubtitle}</p>}{video?.url&&!play&&!failed&&<button className="route-v2-play" type="button" onClick={()=>setPlay(true)}>▶ {video.durationSeconds?mediaCopy.seconds(video.durationSeconds):mediaCopy.play}</button>}<div className="route-v2-meta"><span>{start}</span>{trip.duration&&<span>{trip.duration}</span>}{trip.minimumGuests&&<span>{trip.minimumGuests}–{trip.maximumGuests??''} 人</span>}</div></div></section>; }
...
function RouteV2Spot({spot,index,copy,mediaCopy,fallbackImage}:{spot:TripTimelineItem;index:number;copy:(typeof routeV2Copy)['zh-CN'];mediaCopy:(typeof routeV2MediaCopy)['zh-CN'];fallbackImage:string}) { const [expanded,setExpanded]=useState(false); const short=spot.shortDescription||spot.detail; const long=spot.longDescription; return <article className="route-v2-spot"><header><div><span>SPOT {String(index+1).padStart(2,'0')}</span><h2>{spot.title}</h2>{spot.subtitle&&<small>{spot.subtitle}</small>}</div>{spot.stayMinutes&&<b>{spot.stayMinutes} {copy.minutes}</b>}</header><div className="route-v2-spot-media">{spot.video?.url?<SpotVideoPlayer url={spot.video.url} posterUrl={spot.video.posterUrl||spot.imageUrl||fallbackImage} title={spot.title} playLabel={spot.videoLabel||mediaCopy.play} unavailableLabel={mediaCopy.unavailable}/>:<img src={spot.imageUrl||fallbackImage} alt="" loading="lazy"/>}</div>{short&&<p>{short}</p>}{long&&<><button type="button" onClick={()=>setExpanded(value=>!value)}>{expanded?copy.hide:copy.details}</button>{expanded&&<div className="route-v2-spot-extra">{long}</div>}</>}</article>; }
export function AppTrip() {
  const now=useCurrentTime();
  const resolvedTrip = travelRepository.getTrip(useParams().slug || "");
  const location=useLocation();
  const { departures,state } = useApp();
  const locale=state.ui.locale ?? 'zh-CN';
  const r=passengerRoutesCopy[locale];

  if (!resolvedTrip) return <Empty title={r.notFound} />;
  const t: Trip = resolvedTrip;
  ...
  const localizedTimeline=localizedPublishedTimeline(locale,t);
  const fallbackSpots=displayTrip.stops.map((name,index)=>({name,location:displayTrip.region,intro:routeText.spot(name),history:'',highlights:[] as string[],tip:'',time:localizedTimeline[index]?.time,imageUrl:localizedTimeline[index]?.imageUrl,stayMinutes:localizedTimeline[index]?.stayMinutes,video:localizedTimeline[index]?.video}));
  const publishedSpots=localizedTimeline.map(item=>({name:item.title,location:item.location||displayTrip.region,intro:item.detail,history:'',highlights:item.highlights??[],tip:item.tip??'',time:item.time,imageUrl:item.imageUrl,stayMinutes:item.stayMinutes,video:item.video}));
...
export function Orders() {
  ...
  const [remote, setRemote] = useState<{
    ...
    rows: Array<{
      id: string;
      departure_id: string;
      departure?:{departs_at:string;status:string;trip:{slug:string}|null}|null;
      seat_count: number;
      status: string;
      manual_payment_due_at?: string | null;
    }>;
  ...
  useEffect(() => {
    if (services)
      void Promise.all([
        ...
        void Promise.all(result.data.map(async order=>[order.id,await services.loadOwnOrderBilling(order.id)] as const)).then(entries=>setBillingById(Object.fromEntries(entries.filter(([,value])=>value)) as typeof billingById)).catch(()=>{});
    ...
    <>
      ...
      {services ? (
        remote.loading ? (
          ...
          <>{remote.rows.filter(o=>matchesFilter(o)).map(o=>{
            const departure=departures.find(d=>d.id===o.departure_id);
...
export function OrderDetail() {
  ...
  const returnTo=requestedReturn?.startsWith('/app/orders?')?requestedReturn:'/app/orders';
  ...
  useEffect(() => {
    ...
    }).catch(error=>{if(active){setLoadError(error instanceof Error?error.message:'订单读取失败');setRemoteResolved(true)}});
    return () => {
      active = false;
    };
  }, [services, id,reload]);
  ...
  if (services) {
    if(loadError)return <div role="alert"><h1>订单读取失败</h1><p>{loadError}</p><button onClick={()=>{setLoadError('');setRemoteResolved(false);setReload(n=>n+1)}}>重新读取</button><Link to={returnTo}>返回订单列表</Link></div>;
    if (!remoteResolved)
      return (
        <Empty title={c.loading} text={c.wait} />
      );
    ...
    return (
      <>
        ...
        {billing&&<section className="receipt" aria-label="不可变订单账单"><h2>订单费用明细</h2>{(billing.lineItems??[]).map((item,index)=><div key={`${item.kind}-${index}`}><span>{item.label}{item.quantity>0?` · ${item.quantity}项`:''}</span><b>{item.amountJpy<0?'-':''}¥{Math.abs(item.amountJpy).toLocaleString('ja-JP')}</b></div>)}<div><span>订单原价</span><b>{typeof billing.grossAmountJpy==='number'?`¥${billing.grossAmountJpy.toLocaleString('ja-JP')}`:c.pending}</b></div>{billing.discountAmountJpy>0&&<div><span>优惠券抵扣（仅优惠一席）</span><b>-¥{billing.discountAmountJpy.toLocaleString('ja-JP')}</b></div>}<div><span>实际应付</span><b>{typeof billing.amountPaidJpy==='number'?`¥${billing.amountPaidJpy.toLocaleString('ja-JP')}`:c.pending}</b></div><div><span>付款方式</span><b>{billing.paymentKind==='coupon_covered'?'优惠券抵扣，无需付款':billing.paymentKind==='bank_transfer'?'银行转账':billing.paymentKind==='stripe'?'银行卡支付':'待确认'}</b></div>{billing.userConfirmedAt&&<div><span>确认费用时间</span><b>{new Date(billing.userConfirmedAt).toLocaleString(locale,{timeZone:'Asia/Tokyo'})}</b></div>}{billing.refunds.length>0&&<details><summary>退款流水（{billing.refunds.length}）</summary>{billing.refunds.map((refund,index)=><div key={index}><span>{refund.channel} · {refund.status}</span><b>{refund.amountJpy==null?'金额核对中':`¥${refund.amountJpy.toLocaleString('ja-JP')}`}</b></div>)}</details>}{!billing.snapshotAvailable&&<p className="notice">这是旧订单，部分历史合同字段当时尚未建立；系统不会使用当前价格补写旧账单。</p>}</section>}
...
const referralPanelCopy={
  'zh-CN':{title:'邀请好友，新人享首单优惠',intro:'好友首单可享 {percent}% 优惠；获批推广大使可获得新人首笔符合资格订单基础票价10%的现金佣金，行程完成且无退款争议后解锁。',copy:'复制邀请链接',share:'分享给好友',copied:'邀请链接已复制',invites:'成功邀请',coupons:'我的优惠券',none:'暂无优惠券',valid:'有效至',pending:'等待符合使用条件',tripStarts:'关联行程时间',available:'预计可使用',invalid:'已失效（订单已取消或退款）',loading:'正在读取邀请权益…'},
  'zh-TW':{title:'邀請好友，新人享首單優惠',intro:'好友首單可享 {percent}% 優惠；核准推廣大使可獲新人首筆合資格訂單基本票價10%現金佣金，行程完成且無退款爭議後解鎖。',copy:'複製邀請連結',share:'分享給好友',copied:'邀請連結已複製',invites:'成功邀請',coupons:'我的優惠券',none:'暫無優惠券',valid:'有效至',pending:'等待符合使用條件',tripStarts:'關聯行程時間',available:'預計可使用',invalid:'已失效（訂單已取消或退款）',loading:'正在讀取邀請權益…'},
  ...
  en:{title:'Invite a friend for a first-booking discount',intro:'Your friend gets {percent}% off their first trip. Approved ambassadors earn 10% cash commission on the eligible base fare of the friend’s first qualifying order, unlocked after completion with no refund dispute.',copy:'Copy invite link',share:'Share',copied:'Invite link copied',invites:'Successful invites',coupons:'My coupons',none:'No coupons yet',valid:'Valid until',pending:'Waiting for eligibility',tripStarts:'Related trip starts',available:'Expected availability',invalid:'Invalid (booking cancelled or refunded)',loading:'Loading referral benefits…'},
...
const referralAchievementLevels={
  'zh-CN':[{at:0,name:'旅行推荐人',benefit:'可使用固定推荐链接；游客推广资格需运营审批'},{at:1,name:'有效推荐',benefit:'获批后按新人首笔有效订单基础票价计10%现金佣金'}],
  'zh-TW':[{at:0,name:'旅行推薦人',benefit:'可使用固定推薦連結；旅客推廣資格需營運審批'},{at:1,name:'有效推薦',benefit:'核准後按新人首筆有效訂單基本票價計10%現金佣金'}],
  ja:[{at:0,name:'旅行紹介者',benefit:'固定紹介リンクを利用できます。旅行者の資格は運営審査が必要です'},{at:1,name:'有効な紹介',benefit:'承認後、初回対象注文の基本料金の10%を現金コミッションとして計上'}],
  en:[{at:0,name:'Travel referrer',benefit:'Use a fixed referral link; passenger ambassador access requires approval'},{at:1,name:'Qualified referral',benefit:'After approval, earn 10% cash commission on the eligible base fare of the first qualifying order'}],
  es:[{at:0,name:'Referente de viajes',benefit:'Usa un enlace fijo; la condición de embajador requiere aprobación'},{at:1,name:'Recomendación válida',benefit:'Tras la aprobación, comisión en efectivo del 10% sobre la tarifa base elegible del primer pedido'}],
  vi:[{at:0,name:'Người giới thiệu',benefit:'Dùng liên kết cố định; đại sứ là du khách cần được duyệt'},{at:1,name:'Giới thiệu hợp lệ',benefit:'Sau khi duyệt, nhận 10% tiền mặt trên giá vé cơ bản đủ điều kiện của đơn đầu tiên'}],
  ne:[{at:0,name:'यात्रा सिफारिसकर्ता',benefit:'स्थायी लिङ्क प्रयोग गर्नुहोस्; यात्रु एम्बेसडरलाई स्वीकृति चाहिन्छ'},{at:1,name:'योग्य सिफारिस',benefit:'स्वीकृतिपछि पहिलो योग्य अर्डरको आधार भाडामा १०% नगद कमिसन'}],
  ko:[{at:0,name:'여행 추천인',benefit:'고정 추천 링크 사용; 여행객 앰배서더는 운영 승인이 필요합니다'},{at:1,name:'유효 추천',benefit:'승인 후 첫 유효 주문의 기본 운임에 대해 10% 현금 커미션 적립'}],
} as const;
...
export function Profile() {
 ...
 const [status,setStatus]=useState<'loading'|'ready'|'saving'|'unavailable'>(services?'loading':'unavailable');
 const [notice,setNotice]=useState('');
 useEffect(()=>{
  let active=true;
  if(!services)return;
 ...
 async function save(event:FormEvent<HTMLFormElement>){
  ...
  if(!services||status!=='ready'||!salutation)return;
  const form=new FormData(event.currentTarget);
  const name=composePassengerDisplayName(displayName,salutation);
 ...
 return <div className="passenger-profile-sections">
  ...
  <section className="form" aria-labelledby="personal-details-title">
   ...
   <p className="privacy">{pc.logoutText}</p>
  </section>
 </div>;
}
