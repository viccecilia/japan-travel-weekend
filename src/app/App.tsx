...
function RouteDetailV2({trip, locale, selectedDeparture}: {trip: Trip; locale: PassengerLocale; selectedDeparture: {id:string;departureTime?:string|null;expectedEndTime?:string|null}|null}) {
  const copy=routeV2Copy[locale]; const local=trip.localizedContent?.[locale]??trip.localizedContent?.['zh-CN']??{}; const text=(key:string, fallback='')=>v2Text(local[key],fallback); const heroTitle=text('heroTitle',trip.heroTitle||text('title',trip.shortTitle)); const heroSubtitle=text('heroSubtitle',trip.heroSubtitle||trip.summary); const highlight=text('heroHighlightPhrase',trip.heroHighlightPhrase||''); const titleParts=highlight&&heroTitle.includes(highlight)?heroTitle.split(highlight):[heroTitle]; const timeline=localizedPublishedTimeline(locale,trip); const routeStops=timeline.map(item=>item.title).filter(Boolean); const start=text('departureCity',trip.departureCity||trip.region); const journey=[start,...routeStops,start].filter(Boolean); const departureTime=selectedDeparture?.departureTime?new Intl.DateTimeFormat(locale,{timeZone:'Asia/Tokyo',hour:'2-digit',minute:'2-digit'}).format(new Date(selectedDeparture.departureTime)):''; const returnTime=selectedDeparture?.expectedEndTime?new Intl.DateTimeFormat(locale,{timeZone:'Asia/Tokyo',hour:'2-digit',minute:'2-digit'}).format(new Date(selectedDeparture.expectedEndTime)):''; const notices=[['bookingNotice',copy.booking,text('bookingNotice',trip.bookingNotice||trip.notices[0]||'')],['cancellationPolicy',copy.cancellation,text('cancellationPolicy',trip.cancellationPolicy)],['participantRules',copy.participants,text('participantRules',trip.participantRules||trip.childPolicy)],['weatherNotice',copy.weather,text('weatherNotice',trip.weatherNotice||trip.weatherPolicy)],['baggageNotice',copy.baggage,text('baggageNotice',trip.baggageNotice||trip.luggagePolicy)],['safetyNotice',copy.safety,text('safetyNotice',trip.safetyNotice)]] as const; const preparation=(Array.isArray(local.preparation)?local.preparation:trip.preparation?.length?trip.preparation:trip.packingList).filter((item):item is string=>typeof item==='string'&&Boolean(item.trim()));
  return <div className="route-detail-v2"><RouteV2Hero trip={trip} heroTitle={heroTitle} heroSubtitle={heroSubtitle} titleParts={titleParts} highlight={highlight} start={start} mediaCopy={routeV2MediaCopy[locale]}/><section className="route-v2-section"><span className="route-v2-kicker">TODAY'S ROUTE</span><h2>{copy.today}</h2><div className="route-v2-chain"><div>{journey.map((item,index)=><span key={`${item}-${index}`}>{index>0&&<i>→</i>}{item}</span>)}</div>{(departureTime||returnTime)&&<small>{departureTime&&`${departureTime} ${copy.departure}`}{departureTime&&returnTime&&' · '}{returnTime&&`${copy.returning} ${returnTime}`}</small>}</div></section><section className="route-v2-section route-v2-spots">{timeline.map((spot,index)=><RouteV2Spot key={spot.id??`${spot.title}-${index}`} spot={spot} index={index} copy={copy} mediaCopy={routeV2MediaCopy[locale]} fallbackImage={trip.heroImage}/>)}</section><section className="route-v2-section"><span className="route-v2-kicker">DAY SCHEDULE</span><h2>{copy.schedule}</h2><div className="route-v2-timeline">{timeline.map((item,index)=><article key={item.id??`${item.title}-${index}`}><time>{item.time||'—'}</time><i/><div><b>{item.title}</b><p>{[item.detail,item.stayMinutes?`${item.stayMinutes} ${copy.minutes}`:'',item.type].filter(Boolean).join(' · ')}</p></div></article>)}</div></section>{(trip.included.length||trip.excluded.length||preparation.length>0)&&<section className="route-v2-section"><span className="route-v2-kicker">BEFORE YOU GO</span><h2>{copy.fees}</h2><div className="route-v2-info-grid">{trip.included.length>0&&<RouteV2List title={copy.included} items={trip.included}/>} {trip.excluded.length>0&&<RouteV2List title={copy.excluded} items={trip.excluded}/>} {preparation.length>0&&<RouteV2List title={copy.preparation} items={preparation}/>}</div></section>}{notices.some(([, ,body])=>body)&&<section className="route-v2-section"><span className="route-v2-kicker">TRAVEL NOTES</span><h2>{copy.notes}</h2><div className="route-v2-notices">{notices.filter(([, ,body])=>body).map(([key,label,body])=><details key={key}><summary>{label}<span>＋</span></summary><p>{body}</p></details>)}</div></section>}<section className="route-v2-cta"><small>READY WHEN YOU ARE</small><h3>{copy.ready}</h3><Link to={`/app/booking/${trip.slug}${selectedDeparture?`?departureId=${encodeURIComponent(selectedDeparture.id)}`:''}`}>{copy.book}</Link></section></div>;
}
