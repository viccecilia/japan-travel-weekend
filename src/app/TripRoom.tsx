...
type RemoteMessage = {id:string;content:string;original_content?:string|null;source_language?:string;template_key?:string|null;important?:boolean;author_id?:string;created_at?:string;trip_room_message_translations?:Array<{target_language:string;translated_content:string;provider:string;quality:string}>};
...
function RemoteTripRoom({ services }: { services: ProductionBrowserServices }) {
  ...
  if(passenger){
    const remoteStop={id:`departure-${room.departure_id}`,name:'当前集合',meetingTime:room.departs_at?new Date(room.departs_at).toLocaleTimeString('zh-CN',{timeZone:'Asia/Tokyo',hour:'2-digit',minute:'2-digit',hour12:false}):'00:00',meetingPointName:room.meeting_name??'集合地点待司导更新',meetingPointDescription:room.meeting_address??'集合地点说明尚未发布',latitude:room.map_lat==null?Number.NaN:Number(room.map_lat),longitude:room.map_lng==null?Number.NaN:Number(room.map_lng),status:'current' as const};
    const passengerMessages=messages.map(message=>({id:message.id,senderId:message.author_id??'operations',name:message.author_id===currentUserId?'我':message.template_key?'司导通知':message.author_id?'本车成员':'Japan Travel',role:(message.template_key?'guide':message.author_id?'passenger':'operations') as 'passenger'|'guide'|'operations',content:message.original_content??message.content,translated:translatedMessage(message)??undefined,sourceLanguage:message.source_language,time:message.created_at?new Date(message.created_at).toLocaleTimeString('zh-CN',{timeZone:'Asia/Tokyo',hour:'2-digit',minute:'2-digit',hour12:false}):''}));
    return <PassengerChatRoom tripName={room.vehicle_label??'本车旅行团'} status={room.room_status==='closed'?'ended':room.room_status==='open'?'meeting':'preparing'} stage={room.room_status==='open'?'集合与登车':'出发准备'} meetingActive={messages.some(message=>message.template_key==='vehicle_arrived'||message.template_key==='return_vehicle')} party={{arrived:attendanceTotals.arrived,total:attendanceTotals.total||room.booked_seats,distanceMeters:null,walkMinutes:null}} guide={{name:'当班司导',role:'司导资料待更新',phone:'',avatar:'导',vehicle:{type:room.vehicle_type||'车型待定',color:'颜色待更新',plate:room.vehicle_label??'车牌待更新'}}} stops={[remoteStop]} messages={passengerMessages} readOnly={!access.enabled} onSend={content=>void services.tripRoom.sendMessage(room.room_id,content)} onArrive={()=>{const own=attendance[0];if(own)void updateOwnAttendance(own.passenger_id,'at_meeting_point')}}/>;
  }
  return (
