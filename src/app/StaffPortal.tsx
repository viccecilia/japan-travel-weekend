...
function JourneyAction({task,preview}:{task:StaffTask;preview:boolean}){
  ...
  const locationSession=useRef<string>(crypto.randomUUID());
  ...
  const transition=async(action:'stop_arrived'|'trip_completed')=>{
    ...
    if(value&&action==='trip_completed'&&locationWatch.current!==null){navigator.geolocation.clearWatch(locationWatch.current);locationWatch.current=null}
  ...
  const shareLocation=async()=>{
    ...
    setBusy('location');const session=preview||!services?crypto.randomUUID():await services.startStaffLocation(task.vehicle_group_id);if(!session){setBusy('');setResult('无法开启定位会话，请检查任务、群聊和工作人员权限。');return}locationSession.current=session;locationSequence.current=0;locationWatch.current=navigator.geolocation.watchPosition(async position=>{if(locationSending.current)return;locationSending.current=true;locationSequence.current+=1;const ok=preview||!services?true:await services.publishStaffLocation(task.vehicle_group_id,locationSession.current,position.coords.latitude,position.coords.longitude,position.coords.accuracy,new Date(position.timestamp).toISOString(),locationSequence.current);locationSending.current=false;setBusy('');setResult(ok?`位置持续采集中；最近采集 ${new Date(position.timestamp).toLocaleTimeString('zh-CN')}，精度约 ${Math.round(position.coords.accuracy)} 米。浏览器进入后台或锁屏后可能暂停。`:'位置上报失败，请检查定位权限、网络和当前任务。')},()=>{setBusy('');setResult('无法读取当前位置，请允许浏览器定位后重试。')},{enableHighAccuracy:true,timeout:15000,maximumAge:10000});
  };
  return <section className="staff-detail"><div className="staff-detail-note">到达景点会写入本车时间线并通知游客。开始自由活动会向已付款游客发送随行提醒；结束行程会停止定位并将群聊转为只读。</div>{routeStops.length>0&&<div className="staff-template-grid" aria-label="今日线路节点">{routeStops.map(stop=><button type="button" key={stop.id} className={stopName===stop.name?'active':''} onClick={()=>setStopName(stop.name)}><b>{stop.meetingTime}</b><span>{stop.name}</span><small>{stop.meetingPointName}</small></button>)}</div>}<form className="staff-escalation" onSubmit={e=>{e.preventDefault();void transition('stop_arrived')}}><label>当前到达景点<input required minLength={2} maxLength={160} value={stopName} onChange={e=>setStopName(e.target.value)}/></label><label>现场说明<textarea required minLength={3} maxLength={300} value={reason} onChange={e=>setReason(e.target.value)}/></label><button disabled={busy!==''}>确认到达景点</button></form><div className="staff-template-grid"><label>自由活动分钟<input type="number" min={5} max={360} value={freeMinutes} onChange={e=>setFreeMinutes(Number(e.target.value))}/></label><button type="button" disabled={busy!==''||stopName.trim().length<2} onClick={async()=>{if(!window.confirm(`确认在“${stopName.trim()}”开始约 ${freeMinutes} 分钟自由活动？`))return;setBusy('free-time');const ok=preview||!services?true:await services.startStaffFreeTime(task.vehicle_group_id,stopName.trim(),freeMinutes);setBusy('');setResult(ok?'自由活动已开始，已付款游客将收到 AI 随行与集合提醒。':'操作失败，请确认群聊已开放和工作人员权限。')}}>{busy==='free-time'?'正在发布':'开始自由活动'}</button><button type="button" disabled={busy!==''} onClick={shareLocation}>{busy==='location'?'正在读取定位':'开始共享行程位置'}</button><button type="button" disabled={busy!==''} onClick={()=>void stopLocation()}>停止位置共享</button><button type="button" disabled={busy!==''||reason.trim().length<3} onClick={()=>void transition('trip_completed')}>{busy==='trip_completed'?'正在结束':'结束本车行程'}</button></div>{result&&<p className="staff-result" role="status">{result}</p>}</section>
}
