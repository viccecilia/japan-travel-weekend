...
export function Discover() {
 ...
 const change=(offset:number)=>{if(visible.length)setActiveId(visible[(index+offset+visible.length)%visible.length].id);setTouchCount(n=>n+1)};
 return <section className="discover-hero" aria-label={c.discover} onDragStart={event=>event.preventDefault()}
   ...
   {hero ? <>
     ...
     <div key={hero.id+':'+touchCount} className="discover-title">
       <h1>{discoverText(hero,locale,product).title}</h1>
       <p>{discoverText(hero,locale,product).subtitle}</p>
     </div>
     {visible.length>1&&<>
       <button className="discover-edge discover-previous discover-control" aria-label={c.previous} onClick={()=>change(-1)}>‹</button>
       <button className="discover-edge discover-next discover-control" aria-label={c.next} onClick={()=>change(1)}>›</button>
     </>}
     ...
     <div className="discover-dots discover-control" aria-label={c.discover}>
       {visible.map((item,i)=><button key={item.id} aria-label={String(i+1)} aria-current={i===index?'true':undefined} onClick={()=>{setActiveId(item.id);setTouchCount(n=>n+1)}}><span/></button>)}
     </div>
