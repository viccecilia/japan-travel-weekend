...
export function ProductCenter() {
  ...
  const [copySource,setCopySource]=useState<Pick<OperationsProduct,"id"|"title"|"slug"|"catalogVersion">|null>(null);
  ...
  const openCopy=()=>{if(!selected)return;setCopySource({id:selected.id,title:selected.title,slug:selected.slug,catalogVersion:selected.catalogVersion});setCopySlug(`${selected.slug}-copy`);setCopyTitle(`${selected.title} 副本`);setCopyOpen(true)};
  const closeCopy=()=>{setCopyOpen(false);setCopySource(null)};
  const copyProduct=async(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();if(!services||!copySource||busy)return;setBusy(true);try{const result=await services.operations.copyProduct({sourceId:copySource.id,sourceVersion:copySource.catalogVersion,slug:copySlug.trim(),title:copyTitle.trim()});setNotice(result.ok?'已复制并进入新产品草稿。':`复制失败：${result.error??'请检查网络后重试'}`);if(result.ok){closeCopy();await reload(result.id)}}catch(error){setNotice(`复制失败：${error instanceof Error?error.message:'网络异常，请稍后重试'}`)}finally{setBusy(false)}};
    ...
    <main className="operations-page">
      ...
      {selected && (
        <section className="operations-section">
          ...
          {copyOpen&&copySource&&<form className="operations-controls operations-inline-editor" aria-label="复制产品" onSubmit={copyProduct}>
            <header><div><span>复制产品</span><h3>建立独立草稿</h3></div><small>固定来源：{copySource.title} · 数据版本 {copySource.catalogVersion}；切换列表不会改变复制来源</small></header>
            ...
            <div className="operations-task-actions"><button className="button" disabled={busy}>确认复制</button><button className="button secondary" type="button" disabled={busy} onClick={closeCopy}>取消</button></div>
          </form>}
