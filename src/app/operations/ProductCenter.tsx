...
export function ProductCenter() {
  ...
  const copyProduct=async(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();if(!services||!selected||busy)return;setBusy(true);try{const result=await services.operations.copyProduct({sourceId:selected.id,sourceVersion:selected.catalogVersion,slug:copySlug.trim(),title:copyTitle.trim()});setNotice(result.ok?'已复制并进入新产品草稿。':`复制失败：${result.error}`);if(result.ok){setCopyOpen(false);await reload(result.id)}}finally{setBusy(false)}};
