...
type UploadTarget = {kind: 'hero'} | {kind: 'heroVideoPoster'} | {kind: 'gallery'; index?: number} | {kind: 'stop'; editorId: string} | {kind: 'stopPoster'; editorId: string};
...
function MediaPanel({draft, update, upload, uploadHeroVideo, uploadBusy, uploadNotice, heroVideoState}: {draft: ProductDraft; update: (patch: Partial<ProductDraft>) => void; upload: (file: File, target: UploadTarget) => Promise<void>; uploadHeroVideo: (file: File) => Promise<void>; uploadBusy: boolean; uploadNotice: string; heroVideoState?: VideoUploadState}) {
  ...
  return <div className="product-editor-stack">
    ...
    <section className="product-editor-card product-spot-video-editor" aria-label="路线 Hero 视频"><header><div><h2>路线 Hero 视频</h2><p>MP4（H.264/AAC）最多 50MB；默认以横版封面呈现，点击后才播放。</p></div><span>草稿媒体</span></header>
...
function ItineraryPanel({draft, update, upload, uploadVideo, uploadBusy, videoUploads}: {draft: ProductDraft; update: (patch: Partial<ProductDraft>) => void; upload: (file: File, target: UploadTarget) => Promise<void>; uploadVideo: (file: File, item: ProductEditorStop) => Promise<void>; uploadBusy: boolean; videoUploads: Record<string, VideoUploadState>}) {
  ...
  const add = () => { const id = crypto.randomUUID(); update({itinerary: [...draft.itinerary, {id, editorId: `stop-${id}`, title: '新景点', description: ''}]}); };
  return <section className="product-editor-card"><header><div><h2>景点行程</h2><p>稳定景点标识保证改名不丢焦点；排序会同步到游客预览。</p></div><span>{draft.itinerary.length} 个景点</span></header>
    <div className="product-itinerary-list">{draft.itinerary.map((item, index) => {
      ...
      const videoState = videoUploads[item.editorId];
      const videoUrl = videoState?.localUrl ?? item.video?.url;
      return <details key={item.editorId} open={index === 0}><summary><span>{index + 1}</span><b>{title}</b><small>{Number(item.stayMinutes) > 0 ? `${Number(item.stayMinutes)} 分钟` : '未设置停留时间'}</small></summary><div className="product-itinerary-fields">
        <div className="product-editor-fields-two"><Field label="景点名称"><input value={String(item.title ?? item.name ?? '')} onChange={(event) => change(item.editorId, {title: event.target.value})} /></Field><Field label="地点"><input value={String(item.location ?? '')} onChange={(event) => change(item.editorId, {location: event.target.value})} /></Field></div>
        ...
        <Field label="短介绍（默认展示）"><textarea value={String(item.shortDescription ?? item.description ?? '')} onChange={(event) => change(item.editorId, {shortDescription: event.target.value, description: event.target.value})} /></Field><Field label="详细介绍（可展开）"><textarea value={String(item.longDescription ?? '')} onChange={(event) => change(item.editorId, {longDescription: event.target.value})} /></Field><div className="product-editor-fields-two"><Field label="景点短标签"><input value={String(item.subtitle ?? '')} onChange={(event) => change(item.editorId, {subtitle: event.target.value})} /></Field><Field label="节点类型"><input value={String(item.type ?? 'spot')} onChange={(event) => change(item.editorId, {type: event.target.value})} /></Field></div><Field label="游览提示"><textarea value={String(item.tip ?? '')} onChange={(event) => change(item.editorId, {tip: event.target.value})} /></Field>
        <div className="product-stop-media">{item.imageUrl && <img src={String(item.imageUrl)} alt="景点图片预览" />}<label className="button secondary">上传该景点图片<input aria-label={`上传${title}图片`} type="file" accept="image/jpeg,image/png,image/webp" disabled={uploadBusy} onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(file, {kind: 'stop', editorId: item.editorId}); event.currentTarget.value = ''; }} /></label><Field label="或使用已有图片地址"><input value={String(item.imageUrl ?? '')} onChange={(event) => change(item.editorId, {imageUrl: event.target.value})} /></Field></div>
        <section className="product-spot-video-editor" aria-label={`${title}视频`}><header><h3>景点视频</h3><span>MP4 · H.264/AAC · 最大50MB</span></header>
          {videoUrl && <video controls preload="metadata" playsInline poster={item.video?.posterUrl || String(item.imageUrl ?? '') || undefined} src={videoUrl} />}
          {videoState && <><div className="product-video-progress" aria-label={`上传进度 ${videoState.progress}%`}><i style={{width: `${videoState.progress}%`}} /></div><p className={videoState.failed ? 'product-video-error' : 'product-video-note'} role="status">{videoState.message}</p></>}
          {!videoState && <p className="product-video-note">上传完成后才写入持久地址；保存草稿后仍不会影响游客公开版本。</p>}
          <div className="product-spot-video-actions"><label className="button secondary">{item.video ? '更换视频' : '上传视频'}<input aria-label={`${item.video ? '更换' : '上传'}${title}视频`} type="file" accept="video/mp4" disabled={uploadBusy || (!!videoState && !videoState.failed && videoState.progress < 100)} onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadVideo(file, item); event.currentTarget.value = ''; }} /></label>
            <label className="button secondary">上传封面<input aria-label={`上传${title}视频封面`} type="file" accept="image/jpeg,image/png,image/webp" disabled={uploadBusy || !item.video} onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(file, {kind: 'stopPoster', editorId: item.editorId}); event.currentTarget.value = ''; }} /></label>
            {item.video && <button type="button" onClick={() => change(item.editorId, {video: undefined})}>移除视频引用</button>}
          </div>
        </section>
...
function RulesPanel({draft, update}: {draft: ProductDraft; update: (patch: Partial<ProductDraft>) => void}) {
  return <div className="product-editor-stack"><section className="product-editor-card"><header><div><h2>费用与准备</h2><p>每行录入一项，预览与公开版本沿用同一内容字段。</p></div></header><Field label="费用包含"><textarea value={lineText(draft.included)} onChange={(event) => update({included: lines(event.target.value)})} /></Field><Field label="费用不含"><textarea value={lineText(draft.excluded)} onChange={(event) => update({excluded: lines(event.target.value)})} /></Field><Field label="出发准备"><textarea value={lineText(draft.preparation)} onChange={(event) => update({preparation: lines(event.target.value)})} /></Field></section><section className="product-editor-card"><header><div><h2>出行须知</h2><p>空字段不会在游客端显示。</p></div></header><Field label="预订须知"><textarea value={draft.bookingNotice} onChange={(event) => update({bookingNotice: event.target.value})} /></Field><Field label="取消与退款"><textarea value={draft.cancellationPolicy} onChange={(event) => update({cancellationPolicy: event.target.value})} /></Field><Field label="参加规则"><textarea value={draft.participantRules} onChange={(event) => update({participantRules: event.target.value})} /></Field><Field label="天气提醒"><textarea value={draft.weatherNotice} onChange={(event) => update({weatherNotice: event.target.value})} /></Field><Field label="行李说明"><textarea value={draft.baggageNotice} onChange={(event) => update({baggageNotice: event.target.value})} /></Field><Field label="安全提示"><textarea value={draft.safetyNotice} onChange={(event) => update({safetyNotice: event.target.value})} /></Field></section></div>;
}
...
export function ProductEditPage() {
  ...
  const upload = async (file: File, target: UploadTarget) => {
    ...
    setDraft((current) => {
      ...
      return {...current, itinerary: current.itinerary.map((item) => item.editorId === target.editorId ? target.kind === 'stopPoster' && item.video ? {...item, video: {...item.video, posterUrl: localUrl}} : {...item, imageUrl: localUrl} : item)};
    }); setDirty(true);
    let uploaded = false;
    try {
      ...
      setDraft((current) => {
        ...
        return {...current, itinerary: current.itinerary.map((item) => item.editorId === target.editorId ? target.kind === 'stopPoster' && item.video?.posterUrl === localUrl ? {...item, video: {...item.video, posterUrl: result.url!}} : item.imageUrl === localUrl ? {...item, imageUrl: result.url!} : item : item)};
      });
  ...
  const uploadVideo = async (file: File, item: ProductEditorStop) => {
    if (!services || !product || videoUploadBusy) return;
    const validationError = await validateWebReadySpotVideo(file);
    if (validationError) { setVideoUploads((current) => ({...current, [item.editorId]: {progress: 0, message: `上传失败：${validationError}`, failed: true}})); return; }
    const previousLocal = videoUploads[item.editorId]?.localUrl;
    ...
    setVideoUploads((current) => ({...current, [item.editorId]: {progress: 0, message: '正在直传到素材存储…', failed: false, localUrl}}));
    try {
      const stopId = String(item.id ?? item.stopId ?? item.placeId ?? item.editorId.replace(/^stop-/, ''));
      const result = await services.operations.uploadProductSpotVideo(product.id, stopId, file, (progress) => setVideoUploads((current) => ({...current, [item.editorId]: {...current[item.editorId], progress, message: progress < 100 ? `正在上传 ${progress}%…` : '上传完成，等待写入草稿'}})));
      if (result.error || !result.url || !result.storagePath) { setVideoUploads((current) => ({...current, [item.editorId]: {progress: current[item.editorId]?.progress ?? 0, message: `上传失败：${result.error ?? '未返回持久地址'}；可重新选择文件重试。`, failed: true, localUrl}})); return; }
      setDraft((current) => current ? {...current, itinerary: current.itinerary.map((entry) => entry.editorId === item.editorId ? {...entry, video: {url: result.url!, storagePath: result.storagePath!, posterUrl: entry.video?.posterUrl || String(entry.imageUrl ?? '') || undefined, mimeType: 'video/mp4', sizeBytes: file.size}} : entry)} : current);
      setDirty(true);
      setVideoUploads((current) => ({...current, [item.editorId]: {progress: 100, message: '视频上传成功，已加入当前草稿；请保存草稿完成关联。', failed: false}}));
      URL.revokeObjectURL(localUrl); objectUrls.current.delete(localUrl);
    } catch (error) { setVideoUploads((current) => ({...current, [item.editorId]: {progress: current[item.editorId]?.progress ?? 0, message: `上传失败：${error instanceof Error ? error.message : '网络异常'}；可重试。`, failed: true, localUrl}})); }
  };
