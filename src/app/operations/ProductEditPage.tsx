...
export function ProductEditPage() {
  ...
  return <main className="operations-page product-editor-page" data-location={location.pathname}>
    ...
    <footer className="product-editor-savebar"><div><b>{uploadBusy ? '图片正在上传' : dirty ? '有未保存修改' : '草稿内容已保存'}</b><small>版本 {product.catalogVersion}；并发冲突时不会覆盖他人修改</small></div><div className="product-editor-save-actions"><button className="button product-editor-save-primary" type="button" disabled={busy || uploadBusy || !dirty} onClick={() => void save()}>{busy ? '正在保存…' : '保存草稿'}</button><button className="button product-editor-save-primary" type="button" disabled={busy || uploadBusy || dirty || product.draftRevision == null} onClick={() => void publish()}>发布草稿</button><button className="button secondary product-editor-more-toggle" type="button" aria-expanded={mobileMoreOpen} aria-controls="product-editor-more-actions" onClick={() => setMobileMoreOpen((value) => !value)}>更多</button><div id="product-editor-more-actions" className="product-editor-more-actions" data-open={mobileMoreOpen ? 'true' : 'false'}><button className="button secondary" type="button" disabled={busy || uploadBusy || !dirty} onClick={() => { setMobileMoreOpen(false); void reload(product.id); }}>放弃修改</button>{product.status !== 'archived' && <button className="button secondary" type="button" disabled={busy || uploadBusy || dirty} onClick={() => { setMobileMoreOpen(false); void setStatus('archive'); }}>下架</button>}<button className="button secondary" type="button" disabled={busy || uploadBusy} onClick={() => { setMobileMoreOpen(false); if (!dirty || window.confirm('当前修改尚未保存，确定返回吗？')) navigate(-1); }}>返回</button></div></div></footer>
  </main>;
}
