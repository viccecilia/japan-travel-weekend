...
type UploadTarget = {kind: 'hero'} | {kind: 'gallery'; index?: number} | {kind: 'stop'; editorId: string} | {kind: 'stopPoster'; editorId: string};
  ...
  return <section className="product-editor-card"><header><div><h2>景点行程</h2><p>稳定景点标识保证改名不丢焦点；排序会同步到游客预览。</p></div><span>{draft.itinerary.length} 个景点</span></header>
    <div className="product-itinerary-list">{draft.itinerary.map((item, index) => {
      ...
      return <details key={item.editorId} open={index === 0}><summary><span>{index + 1}</span><b>{title}</b><small>{Number(item.stayMinutes) > 0 ? `${Number(item.stayMinutes)} 分钟` : '未设置停留时间'}</small></summary><div className="product-itinerary-fields">
        ...
        <Field label="景点介绍"><textarea value={String(item.description ?? '')} onChange={(event) => change(item.editorId, {description: event.target.value})} /></Field><Field label="游览提示"><textarea value={String(item.tip ?? '')} onChange={(event) => change(item.editorId, {tip: event.target.value})} /></Field>
...
export function ProductEditPage() {
  ...
  const save = async () => {
    if (!services || !product || !draft || busy || uploadBusy || videoUploadBusy) return false; if (!draft.title.trim() || !draft.summary.trim()) { setNotice('保存失败：标题和一句话简介不能为空'); return false; } if ([draft.heroImageUrl, ...draft.gallery, ...draft.itinerary.flatMap((item) => [String(item.imageUrl ?? ''), item.video?.url ?? '', item.video?.posterUrl ?? ''])].some((url) => url.startsWith('blob:'))) { setNotice('保存失败：仍有图片或视频只存在于本地预览，请重新上传成功后再保存。'); return false; } setBusy(true);
