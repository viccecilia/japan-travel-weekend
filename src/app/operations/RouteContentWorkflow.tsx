...
import {draftFromProduct, type ProductDraft} from './productDraft';
...
export function RouteContentWorkflow({product, draft, update, onImportChinese, onReviewLocale}: {product: OperationsProduct; draft: ProductDraft; update: (patch: Partial<ProductDraft>) => void; onImportChinese: (content: ContentPackage) => void; onReviewLocale: (locale: string) => void}) {
  ...
  const translation = useMemo(() => buildRouteTranslationPackage({...product, title: draft.title, content: {...product.content, ...{locales: draft.locales}, summary: draft.summary, description: draft.description, itinerary: draft.itinerary.map(({editorId, ...item}) => item)}}), [product, draft]);
  ...
  const readTranslation = async (file: File) => {
    try { const validated = validateTranslationPackage(await parseJsonFile(file)); setIssues(validated.issues); if (!validated.package) { setNotice('译文包未导入：请先修正上述错误。'); return; } if (validated.package.entity.entity_type !== 'route' || validated.package.entity.entity_id !== product.id) { setNotice('译文包不属于当前路线，已拒绝导入。'); return; } const applied = applyRouteTranslationPackage({...product, title: draft.title, content: {...product.content, locales: draft.locales, summary: draft.summary, description: draft.description, itinerary: draft.itinerary.map(({editorId, ...item}) => item)}}, validated.package); const next = draftFromProduct({...product, content: applied.content}); update(next); setIssues([...validated.issues, ...applied.warnings]); setNotice(`已导入 ${applied.imported} 条译文，跳过 ${applied.skipped} 条。当前为未发布草稿，请检查预览后保存。`); }
