...
export function CreateAccount(){
  ...
  const submit=async(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();const form=new FormData(event.currentTarget);const password=String(form.get('password'));if(!validatePassword(password)){setMessage(passwordRuleText);return}if(password!==String(form.get('confirmPassword'))){setMessage('两次输入的密码不一致');return}const accountType=String(form.get('accountType')) as 'passenger'|'driver'|'guide';const result=await services?.signUp(String(form.get('email')),password,accountType,String(form.get('displayName')),String(form.get('referral')??''));if(!result){setMessage('暂时无法创建账户，请稍后再试。');return}if(result.session&&result.user?.email){setState({...state,user:{email:result.user.email}});nav(accountType==='passenger'?'/app':'/app/account-status',{replace:true});return}setMessage(accountType==='passenger'?'注册申请已提交。请检查邮箱并完成验证；有效推荐码会为双方发放优惠券。':'工作人员申请已提交。完成邮箱验证后，还需要等待后台审核，批准后账户才可进入司导端。')};
...
export function AccountStatus(){
  const {services,setState,state}=useApp();const location=useLocation();const nav=useNavigate();const [destination,setDestination]=useState<string|null>(null);const [busy,setBusy]=useState(false);
  ...
  if(passengerRequired&&destination!=='passenger')return <div className="empty-card" role="alert"><b>当前是工作人员账号，不能用于游客预约</b><p>游客订单与工作人员权限必须使用两个不同账号。系统已停止本次预约，没有进入工作人员页面，也没有提交订单。</p><button className="button full" disabled={busy} onClick={async()=>{setBusy(true);await services?.signOut();setState({...state,user:null});nav(`/app/login?returnTo=${encodeURIComponent(returnTo)}`,{replace:true})}}>{busy?'正在退出…':'退出工作人员账号并登录游客账号'}</button><Link className="text-link" to={accessDestinationPath(destination)}>返回工作人员工作区</Link></div>;
