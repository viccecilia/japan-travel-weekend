...
const createAccount=async role=>{
  ...
  const existing=(await admin.from('profiles').select('id').eq('id',created.id).maybeSingle()).data;
  if(existing)must(await admin.from('profiles').update({role}).eq('id',created.id),`${role} role`);
  else must(await admin.from('profiles').insert({id:created.id,role}),`${role} profile`);
...
async function cleanup(){
  ...
  if(state.groupIds.length){await admin.from('staff_assignments').delete().in('vehicle_group_id',state.groupIds);await admin.from('vehicle_group_orders').delete().in('vehicle_group_id',state.groupIds);await admin.from('vehicle_groups').delete().in('id',state.groupIds)}
  ...
  if(state.orderId){const boarding=(await admin.from('boardings').select('id').eq('order_id',state.orderId)).data??[];if(boarding.length)await admin.from('boarding_verification_attempts').delete().in('boarding_id',boarding.map(row=>row.id));for(const table of ['boardings','fulfilment_work_items','notification_outbox','manual_payment_decisions','payment_events','inventory_locks'])await admin.from(table).delete().eq('order_id',state.orderId);await admin.from('orders').delete().eq('id',state.orderId)}
...
async function cleanupStale(){
  ...
  for(const trip of trips){
    ...
    for(const departure of departures){
      ...
      if(groupIds.length){const rooms=(await admin.from('trip_rooms').select('id').in('vehicle_group_id',groupIds)).data??[];if(rooms.length)await admin.from('trip_room_messages').delete().in('trip_room_id',rooms.map(row=>row.id));await admin.from('trip_rooms').delete().in('vehicle_group_id',groupIds);await admin.from('staff_assignments').delete().in('vehicle_group_id',groupIds);await admin.from('vehicle_group_orders').delete().in('vehicle_group_id',groupIds);await admin.from('vehicle_groups').delete().in('id',groupIds)}
      const orders=(await admin.from('orders').select('id').eq('departure_id',departure.id)).data??[];
      for(const order of orders){const boarding=(await admin.from('boardings').select('id').eq('order_id',order.id)).data??[];if(boarding.length)await admin.from('boarding_verification_attempts').delete().in('boarding_id',boarding.map(row=>row.id));for(const table of ['boardings','fulfilment_work_items','notification_outbox','manual_payment_decisions','payment_events','inventory_locks'])await admin.from(table).delete().eq('order_id',order.id);await admin.from('orders').delete().eq('id',order.id)}
...
try{
  ...
  const cutoff=must(await admin.rpc('process_due_departure_cutoffs',{p_now:now.toISOString()}),'cutoff');if(cutoff?.[0]?.result!=='ready_for_planning')throw new Error('six passengers not ready for planning');
  ...
  console.log(JSON.stringify({ok:true,cutoff:'ready_for_planning',passengers:6,vehicleGroups:state.groupIds.length,driverTaskVisible:true,driverPassengerCount:task.passenger_count,chatStatus:task.room_status,chatOpensAt:task.chat_opens_at,passengerStaffTasks:passengerTasks.length,dispatchStatus:status.dispatch_planning_status,cleanup:'pending'}));
}finally{
  await cleanup();
