...
const departureId=process.argv[2]??'22222222-2222-4222-8222-222222222222';
...
if(confirmed.livemode||confirmed.amount!==100||confirmed.currency!=='jpy'||confirmed.status!=='succeeded')throw new Error(`Unexpected Stripe test result: ${JSON.stringify({livemode:confirmed.livemode,amount:confirmed.amount,currency:confirmed.currency,status:confirmed.status})}`);
...
if(order?.status!=='paid'||order.amount!==100||order.currency!=='JPY'||order.payment_intent_id!==paymentIntentId||hold?.status!=='committed'||eventCount!==1)throw new Error(`Webhook settlement incomplete: ${JSON.stringify({orderStatus:order?.status,amount:order?.amount,holdStatus:hold?.status,eventCount})}`);
