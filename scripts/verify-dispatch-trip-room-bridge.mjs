...
try{
  ...
  departureId=must(await admin.from('departures').insert({trip_id:tripId,departs_at:startsAt.toISOString(),ends_at:endsAt.toISOString(),capacity:19,status:'open',seat_price_jpy:100,currency:'JPY',tax_included:true,minimum_guests:1,sales_open_at:new Date(Date.now()-60_000).toISOString(),sales_close_at:new Date(startsAt.getTime()-10*60*60_000).toISOString(),meeting_name:'TEST Dispatch bridge meeting point',meeting_address:'TEST fictional address for acceptance only',map_lat:34.666944,map_lng:135.506111}).select('id').single(),'departure').id;
  ...
  for(const [suffix,type] of [['v1','vehicle-10'],['v2','vehicle-14']])vehicleIds.push(must(await operations.rpc('operations_create_vehicle',{p_registration:`${prefix}-${suffix}`,p_vehicle_type:type,p_external_dispatch_id:null}),`vehicle ${suffix}`));
  ...
  if(addedLinkedQualification&&driverIds[0])await operations.from('driver_vehicle_qualifications').delete().eq('driver_id',driverIds[0]).eq('vehicle_type_key','vehicle-10');
