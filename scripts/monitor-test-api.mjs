...
const previous=await load();let healthy;try{healthy=await probe()}catch{healthy=false}const transition=applyProbeResult(previous,healthy,3);await save(transition.state);
const result={healthy,event:transition.event,consecutiveFailures:transition.state.consecutiveFailures,incidentOpen:transition.state.incidentOpen,endpoint:parsed.hostname,delivery:'not_configured'};
