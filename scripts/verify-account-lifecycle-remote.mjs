...
try{
  ...
  const {data:trip,error:tripError}=await admin.from('trips').insert({slug,title:'TEST Account Lifecycle',status:'published'}).select('id').single()
  if(tripError)fail('TEST trip creation failed',tripError);tripId=trip.id
  const {data:departure,error:departureError}=await admin.from('departures').insert({trip_id:tripId,departs_at:new Date(Date.now()+7*86400000).toISOString(),capacity:6,status:'open',meeting_name:'TEST Meeting',seat_price_jpy:100}).select('id').single()
