...
049 收紧退款／取消／过期订单的履约退出：订单失去销售资格后会撤销未使用登车凭证、停止仅属于该订单账户的位置共享、移出车辆组并回收车辆占座；Trip Room、定位共享和翻译上下文同时要求仍有已支付／已确认订单。回滚验收覆盖支付成功自动入组、名单生成、幂等支付、退款后撤销凭证和释放占座。该迁移已加入隔离恢复清单，但在获得新的可丢弃 Supabase 验收环境或访问授权前，不宣称已远程执行。

050 为已支付／已确认订单幂等创建初始登车记录，并补齐历史有效订单；“可生成登车凭证”现在同时要求已分车、Trip Room 已开放且登车状态仍可签发，与服务端签发函数保持一致。冻结房间仍可作为只读履约入口，但不会提前向游客展示可用登车凭证。

055 将原先只写在字段中的短期数据期限落实为 service-role 定时清理：到达既定 `retain_until` 的登车核验尝试，以及已经 `expires_at` 的登车令牌摘要、司机临时位置和乘客临时位置授权会被删除。订单、乘客、支付事件、群聊与通知账本不在该自动清理范围内；这些正式业务记录的留存与匿名化期限仍需业务及法律批准。
