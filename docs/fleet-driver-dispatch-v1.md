...
后台的数据源为Supabase运营表，不使用种子数据生成统计：车型数、可用车辆数、可用司机数和待确认派单数均来自当前快照，并显示日本时间的刷新时刻。运营可以录入车辆、柚子车辆ID、司机、柚子司机ID、可驾驶车型、语言和一段有效可用时间；规划器仅使用实际状态为available的车辆，并仅推荐完整覆盖任务时间且车型资格符合的司机。

021迁移新增车型、车辆、司机、车型资格、司机可用时间、派单任务和派单审计表。迁移文件目前只在仓库完成静态审计，尚未执行到远程测试或隔离恢复项目。真实“确认并发送至柚子”按钮仍保持禁用，直到服务端运营鉴权、任务确认事务、Webhook签名和真实柚子测试接口完成；浏览器不会持有柚子令牌。
