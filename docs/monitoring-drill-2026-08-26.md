...
仓库新增恢复目标预检：只接受 URL 与项目 ref 完全一致、且显式标记为可丢弃的隔离项目，并拒绝当前测试项目和已暂停旧项目。20 个迁移文件还会校验为非空且编号连续 `0001-0020`。恢复项目尚未创建，因此 `backup_restore_drill` 仍保持待办；创建属于外部状态变更，必须取得动作时确认后再执行。
