...
- 052 增加运营核账闭环与审计记录。确认到账必须填写不含银行卡号的核账参考号，并再次检查库存占位：占位仍有效才转为 `paid` 并进入自动配车；占位已失效则转为 `payment_review`，由人工处理已收款但无有效库存的异常。驳回必须填写原因，订单取消并释放仍在占用的座位。接口按运营人员与幂等键防重复，普通乘客不能调用或读取核账记录。
- 053 为测试阶段银行转账设定明确付款窗口：服务端以“申请后 24 小时”和“发车前 2 小时”两者较早者为截止时间，距离发车不足 2 小时时关闭银行转账；截止时间同步延长该订单的座位占位，并返回游客端按日本时间展示。测试通知调度任务会先把逾期未到账订单转为 `expired` 并释放座位。该 24 小时／2 小时规则在正式上线前仍需运营批准或改为正式配置。
