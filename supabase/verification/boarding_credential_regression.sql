...
declare
  ...
  v_digests bytea[]:=array[extensions.digest('opaque-a','sha256'),extensions.digest('opaque-b','sha256'),extensions.digest('opaque-c','sha256'),extensions.digest('opaque-d','sha256'),extensions.digest('opaque-e','sha256'),extensions.digest('opaque-f','sha256')];
begin
  ...
  begin perform public.issue_boarding_credential(v_boardings[1],v_group_two,extensions.digest('issue-wrong-group','sha256'),1::smallint,now()+interval '30 minutes');raise exception 'FAIL issue vehicle mismatch accepted';exception when others then if sqlerrm<>'boarding vehicle group mismatch' then raise;end if;end;
  begin perform public.issue_boarding_credential(v_boardings[1],v_group_one,decode('aa','hex'),1::smallint,now()+interval '30 minutes');raise exception 'FAIL short issue digest accepted';exception when others then if sqlerrm<>'invalid boarding credential' then raise;end if;end;
  ...
  for v_index in 1..6 loop
    perform public.issue_boarding_credential(v_boardings[v_index],v_group_one,v_digests[v_index],1::smallint,case when v_index=4 then now()+interval '1 second' else now()+interval '30 minutes' end);
  end loop;
  ...
  select * into v_result from public.verify_boarding_credential(extensions.digest('unknown-opaque-token','sha256'),v_driver,v_group_one,v_prefix||'unknown',now());if v_result.result<>'revoked' or v_result.boarding_id is not null then raise exception 'FAIL unknown digest did not preserve credential privacy';end if;
