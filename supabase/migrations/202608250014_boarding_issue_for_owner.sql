...
create or replace function public.get_own_boarding_eligibility(p_order uuid)
returns table(boarding_id uuid,boarding_status text,vehicle_group_id uuid,room_status text,expires_at timestamptz)
language sql stable security definer set search_path=public,pg_temp as $$
  select b.id,b.status,vg.id,r.status,d.departs_at+interval '8 hours'
  from public.orders o join public.boardings b on b.order_id=o.id
  join public.vehicle_group_orders vgo on vgo.order_id=o.id join public.vehicle_groups vg on vg.id=vgo.vehicle_group_id
  join public.trip_rooms r on r.vehicle_group_id=vg.id join public.departures d on d.id=o.departure_id
...
revoke all on function public.get_own_boarding_eligibility(uuid) from public,anon;
grant execute on function public.get_own_boarding_eligibility(uuid) to authenticated,service_role;
