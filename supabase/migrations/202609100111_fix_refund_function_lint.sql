...
begin
  ...
  insert into public.notification_outbox(event_id,event_type,recipient_id,order_id,necessary,payload,status) select 'refund-completed:'||p_event_id,'refund-completed',ord.account_id,ord.id,true,jsonb_build_object('orderId',ord.id,'amount',p_amount,'fullRefund',v_full,'currency',ord.currency),'pending' from public.orders ord where ord.id=p_order on conflict(event_id) do nothing;
  return true;
end$$;
