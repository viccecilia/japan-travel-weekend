...
returns table(
  ...
  payment_review_count integer,
  payment_blocked_count integer
)
language sql stable security definer set search_path=public,pg_temp as $$
  select sa.id,sa.role::text,vg.id,tr.id,tr.status,d.id,t.title,d.departs_at,
    d.meeting_name,d.meeting_address,d.map_lat,d.map_lng,va.sequence,va.vehicle_type,
    va.vehicle_label,va.capacity,va.booked_seats,
  ...
  left join public.passenger_checkins pc on pc.passenger_id=p.id
  where sa.staff_id=auth.uid() and sa.role in ('driver','guide','operations')
  group by sa.id,vg.id,tr.id,d.id,t.id,va.id
  order by d.departs_at nulls last,va.sequence;
$$;
