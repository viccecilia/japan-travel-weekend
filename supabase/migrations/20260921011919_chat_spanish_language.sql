alter table public.chat_translation_preferences drop constraint chat_translation_preferences_target_language_check;
alter table public.chat_translation_preferences add constraint chat_translation_preferences_target_language_check
  check(target_language in ('zh-CN','zh-TW','ja','en','ko','es','vi','ne'));
...
begin
  if auth.uid() is null or p_target_language is null or p_target_language not in ('zh-CN','zh-TW','ja','en','ko','es','vi','ne')
    then raise exception 'invalid translation preference'; end if;
...
language sql stable security definer set search_path=public,pg_temp as $$
  ...
  where m.id=p_message and p_target_language in ('zh-CN','zh-TW','ja','en','ko','es','vi','ne')
...
begin
  if p_target_language is null or p_target_language not in ('zh-CN','zh-TW','ja','en','ko','es','vi','ne')
