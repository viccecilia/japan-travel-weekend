...
create or replace function public.operations_preview_departure_batch(p_trip uuid,p_start date,p_end date,p_weekdays integer[],p_departure_time time,p_duration_minutes integer,p_price integer,p_capacity integer,p_sales_open timestamptz,p_close_hours integer)
returns jsonb language plpgsql stable security definer set search_path=public,pg_temp as $$
declare result jsonb;
begin
  if not public.is_operations() then raise exception 'operations role required'; end if;
  if p_start>p_end or p_end-p_start>93 or p_duration_minutes not between 60 and 1440 or p_price<1 or p_capacity<1 or p_close_hours<1 then raise exception 'invalid batch'; end if;
...
begin
  if not public.is_operations() then raise exception 'operations role required'; end if;
  digest:=md5(concat_ws('|',p_trip,p_start,p_end,p_weekdays,p_departure_time,p_duration_minutes,p_price,p_capacity,p_sales_open,p_close_hours,p_meeting_name,p_meeting_address));
  ...
  perform public.operations_preview_departure_batch(p_trip,p_start,p_end,p_weekdays,p_departure_time,p_duration_minutes,p_price,p_capacity,p_sales_open,p_close_hours);
  for day in select value::date from generate_series(p_start,p_end,interval '1 day') value where extract(isodow from value)::integer=any(p_weekdays) loop
    ...
    insert into public.departures(trip_id,departs_at,ends_at,capacity,status,seat_price_jpy,sales_open_at,sales_close_at,booking_closes_at,chat_opens_at,minimum_guests,currency,tax_included,meeting_name,meeting_address)
    values(p_trip,depart_at,depart_at+make_interval(mins=>p_duration_minutes),p_capacity,'open',p_price,p_sales_open,depart_at-make_interval(hours=>p_close_hours),depart_at-make_interval(hours=>p_close_hours),depart_at-interval '21 hours',1,'JPY',true,nullif(trim(p_meeting_name),''),nullif(trim(p_meeting_address),''));created:=created+1;
  end loop;
...
revoke all on function public.capture_paid_order_snapshot(),public.operations_preview_departure_batch(uuid,date,date,integer[],time,integer,integer,integer,timestamptz,integer),public.operations_create_departure_batch(uuid,uuid,date,date,integer[],time,integer,integer,integer,timestamptz,integer,text,text),public.operations_update_departure(uuid,integer,integer,integer,timestamptz,timestamptz,text) from public,anon;
grant execute on function public.operations_preview_departure_batch(uuid,date,date,integer[],time,integer,integer,integer,timestamptz,integer),public.operations_create_departure_batch(uuid,uuid,date,date,integer[],time,integer,integer,integer,timestamptz,integer,text,text),public.operations_update_departure(uuid,integer,integer,integer,timestamptz,timestamptz,text) to authenticated,service_role;
