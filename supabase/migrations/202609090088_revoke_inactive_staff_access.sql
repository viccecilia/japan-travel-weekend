...
returns boolean language sql stable security definer set search_path=public,pg_temp as $$
  select exists(
    ...
    join public.driver_resources dr on dr.account_id=sa.staff_id
    where sa.vehicle_group_id=target
      ...
      and (p.role='operations' or dr.status='available')
  )
$$;
...
revoke all on function public.is_group_staff(uuid),public.can_read_assistance_projection(uuid),public.operations_revoke_staff_access(uuid,text),public.operations_review_staff_application(uuid,text,text),public.get_staff_portal_tasks() from public,anon;
grant execute on function public.is_group_staff(uuid),public.can_read_assistance_projection(uuid),public.get_staff_portal_tasks() to authenticated,service_role;
