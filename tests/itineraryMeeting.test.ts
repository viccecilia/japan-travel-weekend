...
import {applyMeetingChange,currentMeetingStop,meetingUrgency,projectItineraryStops,type ItineraryStop} from '../src/shared/services/itineraryMeeting';
...
describe('动态集合点领域逻辑',()=>{
  ...
  it('集合点切换后旧节点变淡且新节点成为当前节点',()=>{const base=stops.map(({status:_,...stop})=>stop);const projected=projectItineraryStops(base,'停车场');expect(projected.currentIndex).toBe(2);expect(projected.stops.map(stop=>stop.status)).toEqual(['completed','completed','current'])});
  it('临时集合点无法匹配时不伪造已完成节点',()=>{const base=stops.map(({status:_,...stop})=>stop);const projected=projectItineraryStops(base,'临时集合点');expect(projected.currentIndex).toBe(-1);expect(projected.stops.every(stop=>stop.status==='upcoming')).toBe(true)});
});
