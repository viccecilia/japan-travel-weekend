...
describe("聊天翻译语言与重要模板", () => {
  it("按手机首选语言选择目标，并对不支持语言回退英文", () => {
    ...
    expect(preferredChatLanguage(["es-MX"])).toBe("es");
    expect(preferredChatLanguage(["fr-FR"])).toBe("en");
  });

  it("八种重要模板都有八语非空内容，简体中文不重复显示译文", () => {
    ...
    for (const translations of Object.values(staffTemplateTranslations)) {
      expect(Object.keys(translations)).toEqual(expect.arrayContaining(["zh-CN", "zh-TW", "ja", "en", "vi", "ne", "ko", "es"]));
