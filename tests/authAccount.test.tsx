...
describe('账户页面',()=>{
  ...
  it('账户页面不再显示账户删除申请板块',async()=>{
    const rpc=vi.fn(()=>({maybeSingle:async()=>({data:null,error:null})}));
    ...
    render(<MemoryRouter initialEntries={['/app/profile']}><AppProvider services={new ProductionBrowserServices(client,undefined)}><Router/></AppProvider></MemoryRouter>);
    await screen.findByRole('button',{name:'退出账户'});
    expect(screen.queryByText('账户删除申请')).not.toBeInTheDocument();
    expect(screen.queryByRole('button',{name:'提交账户删除申请'})).not.toBeInTheDocument();
  });
