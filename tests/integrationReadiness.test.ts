...
import {isStripeTestPublishableKey} from '../src/shared/integrations/stripeClient';
...
describe('真实服务接入准备',()=>{
  ...
  it('只允许批准的公开客户端配置，拒绝 VITE 服务端秘密',()=>{expect(Object.keys(clientEnvironmentContract)).toEqual(['VITE_RUNTIME_MODE','VITE_API_BASE_URL','VITE_ENABLE_SEED_DATA','VITE_SUPABASE_URL','VITE_SUPABASE_PUBLISHABLE_KEY','VITE_STRIPE_PUBLISHABLE_KEY','VITE_GOOGLE_MAPS_BROWSER_KEY']);expect(()=>safeClientConfig({VITE_PAYMENT_SECRET_KEY:'leak'})).toThrow('服务端秘密');expect(Object.keys(serverEnvironmentContract)).toContain('STRIPE_WEBHOOK_SECRET')});
  it('浏览器 Stripe 配置只接受测试公钥',()=>{expect(isStripeTestPublishableKey('pk_test_example')).toBe(true);expect(isStripeTestPublishableKey('pk_live_example')).toBe(false);expect(isStripeTestPublishableKey(undefined)).toBe(false)});
