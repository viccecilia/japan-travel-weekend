...
describe('payment UI safety gate',()=>{
  ...
  it('shows assistance review and gates boarding until payment and allocation',()=>{for(const copy of ['特殊需求审核','履约需求摘要','付款确认并完成车辆分配后开放登车凭证','不会在确认前收取相关附加费用'])expect(localeCopy).toContain(copy);expect(source).toContain("['paid','confirmed'].includes(remoteOrder.status)&&remoteFulfilment?.boarding_ready===true")});
});
