...
it('45席实车可计划40人并保存真实车型容量，不触发确认或通知', async () => {
  const onSave = vi.fn<(tasks: DispatchPlanDraft[]) => Promise<boolean>>(async () => true);
  ...
  expect(screen.getByRole('dialog')).toHaveAttribute('data-size', 'workflow');
  expect(screen.getByLabelText('旅行团人数汇总')).toHaveTextContent('本班总人数40');
  expect(screen.getByLabelText('旅行团人数汇总')).toHaveTextContent('所选实车总容量45');
  expect(screen.getByText(/不确认派单、不公开车辆、不发送通知/)).toBeInTheDocument();
});

it('支持新增下一辆并阻止超过已付款人数、实车容量和重复资源', () => {
  render(<ManualDispatchPanel departure={departure} snapshot={snapshot} busy={false} onSave={vi.fn(async () => false)} />);
