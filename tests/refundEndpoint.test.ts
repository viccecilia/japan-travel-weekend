...
describe('operations refund endpoint',()=>{
  ...
  it('returns a completed refund as completed instead of processing',async()=>{const createRefund=vi.fn();const gateway={loadForOperations:vi.fn(async()=>({requestId:'request-1',orderId:'order-1',paymentIntentId:'pi_1',amount:1000})),prepare:vi.fn(async()=>({operationId:'operation-1',providerIdempotencyKey:'refund:operation-1',status:'completed'}))} as unknown as RefundGateway;const endpoint=new RefundEndpoint(gateway,{available:true,createRefund});await expect(endpoint.post(session,{requestId:'request-1',idempotencyKey:'refund-key-1'})).resolves.toMatchObject({status:200,body:{status:'refund_completed',replayed:true}});expect(createRefund).not.toHaveBeenCalled()});
  it('requires and records manual refund evidence before completion',async()=>{const completeManual=vi.fn(async()=>true);const gateway={loadForOperations:vi.fn(async()=>({requestId:'request-1',orderId:'order-1',paymentIntentId:null,amount:5000})),prepare:vi.fn(async()=>({operationId:'operation-1',providerIdempotencyKey:'refund:operation-1',status:'awaiting_manual_refund'})),completeManual} as unknown as RefundGateway;const endpoint=new RefundEndpoint(gateway,{available:false,createRefund:vi.fn()});await expect(endpoint.post(session,{requestId:'request-1',idempotencyKey:'refund-key-1',manualReference:'BANK-20260910',actualAmount:5000,evidenceNote:'运营核对银行流水'})).resolves.toMatchObject({status:200,body:{status:'refund_completed'}});expect(completeManual).toHaveBeenCalledWith(expect.objectContaining({reference:'BANK-20260910',actualAmount:5000,accountId:'ops-1'}))});
});
