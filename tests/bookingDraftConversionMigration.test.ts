...
describe('booking draft to inventory order conversion',()=>{
  ...
  it('has rollback-only remote regression evidence',()=>{const regression=readFileSync('supabase/verification/booking_draft_conversion_regression.sql','utf8');for(const marker of ['FAIL mismatched seat count accepted','FAIL draft was not linked atomically','FAIL idempotent conversion changed order','FAIL compensation did not reopen draft','rollback;'])expect(regression).toContain(marker)});
});
