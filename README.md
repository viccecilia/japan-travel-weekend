...
首条标准路线与支付前黄金路径见 [docs/route-commerce-golden-path.md](docs/route-commerce-golden-path.md)。当前测试阶段停在可恢复订单草稿，不发起真实扣款，也不使用未确认辅助服务收费。

测试服务栈已批准为 Supabase + Stripe + Google Maps Platform。隔离 Supabase 测试项目已执行当前适用迁移至 028；虚构账户、RLS、聊天翻译偏好、测试 Webhook、浏览器 100 日元 Stripe 测试卡及支付前草稿安全投影均已验收。配置变量见 `.env.example`，操作与安全边界见 `docs/approved-test-service-stack.md`。
